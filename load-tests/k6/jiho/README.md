...
이 준비 도구는 AWS 배포·대시보드 조회·실측을 실행하거나 완료로 표시하지 않는다. 실제 fixture, URL, 계정·비밀번호·세션·CSRF, resource ID와 원시 로그는 `build/k6/ops05/`에만 두고 Git에 넣지 않는다.
...
공식 campaign은 `0.5×→1×→2×` 고정 순서로 각 배수를 3회 실행하고, 최초 유효 FAIL 또는 INCONSISTENT에서 상승을 멈춘다. 2×까지 모두 통과하면 `PASS_AT_MAX`로 판정한다. 1×는 온라인 세션 300명과 알림 이벤트 25건/분이고 배수를 곱한 값이 그대로 부하가 된다. 필요한 fixture 사용자 수는 `(온라인 세션 + MIXED_EVENT_MAX_VUS) × 2`이며 부족하면 시작 전에 실패한다.
