...
부하 시나리오는 계단마다 목표를 올리고 `stage` 태그로 단계별 지표를 따로 담는다. WebSocket 구독과 전송·조회 시나리오는 같은 `WS_READY_DELAY` 기준으로 워밍업과 단계 경계를 공유한다.
...
`rooms.sql`은 실제로 만든 방·계정의 ID를 `chat_k6_fixture_registry`에 같은 트랜잭션으로 기록하고, `cleanup.sql`은 그 registry ID만 지운다. 두 SQL은 `run_id`별 advisory lock을 잡으므로 같은 실행의 seed와 cleanup이 서로의 중간 상태를 보지 않는다. 따라서 제목이 같은 일반 방이나 다른 실행의 데이터는 정리 대상이 아니다. 예약된 fixture 이메일·제목이 registry 없이 이미 존재하면 seed를 중단한다.
...
장기 WebSocket은 계단을 넘어 유지될 수 있다. 따라서 `load_stage_delivery_ms`는 메시지를 **수신한 시점**의 단계로 태그를 붙이고, `load_stage_connect_ms`와 `load_stage_opened`는 연결을 **시작한 시점**의 단계를 유지한다.
