...
이 디렉터리는 Yejin이 소유하며, `/api/games`의 키워드 검색과 혼합 조회 흐름을 측정하는 k6 시나리오를 관리한다. 소스와 결과의 공통 배치 규칙은 [Load Tests](../../README.md)를 따른다. #551 인덱스 판단용 `02`, `08`과 #867 게임 목록 동시 부하용 `09`를 추적한다.
...
| `09-game-list-concurrency.js` | 기본·relation·complex `/api/games`를 일정 비율로 반복 | #867 최종 V1의 동시 HTTP·Slice 계약·오류율 검증 |
...
- `CONCURRENCY`: `09`의 constant-VU 동시성 단계다. 기본값은 `2`다.
- `DURATION`: `09`의 단계별 부하 유지 시간이다. 기본값은 `30s`다.
- `RUN_ID`: `09` 요청 헤더에 넣는 실행 식별자다.
- `PHASE`: `09` 요청 헤더에 넣는 동시성 단계 식별자다.
...
`09-game-list-concurrency.js`는 setup 단계에서 게임 목록 Slice 응답과 theme·mechanism metadata를 확인한 뒤, 기본 목록·relation filter·complex filter를 각각 약 1/3 비율로 요청한다. `/api/games` 응답의 키는 `content`, `hasNext`, `page`, `size`로 고정해 확인하며, 표준 k6 `http_req_failed`를 HTTP 오류·timeout 지표로 사용한다.

App 2대와 PostgreSQL 컨테이너를 함께 기록하려면 저장소 루트에서 다음 runner를 실행한다. runner는 App image의 OCI revision이 `--server-commit`과 같은지 확인하고, fixture manifest·runner/k6 SHA-256·k6/Docker 버전을 결과에 남긴다.

```bash
node scripts/measurements/game-list-concurrency.mjs \
  --base-url http://127.0.0.1:5174 \
  --server-commit <server-commit> \
  --app1-container <app1-container> \
  --app2-container <app2-container> \
...
원본 k6 summary·로그와 최종 JSON은 `build/k6/game-list-867/`에 생성하며 Git에 커밋하지 않는다. 정본 결과는 [#867 동시 부하·자원 검증 결과](../../../docs/measurements/k6/yejin/game-list-concurrency-2026-08-20.md)에 기록한다.
