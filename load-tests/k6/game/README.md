...
인덱스 비교 실행기는 DB 상태를 직접 바꾸지 않는다. disposable PostgreSQL에서 상태를 준비한 뒤, 같은 `BASE_URL`, `PROFILE`, fixture로 상태별로 한 번씩 실행한다. 아래 다섯 필수값과 스크립트 checksum, k6 버전, 인증 여부가 기존 manifest와 다르면 두 번째 실행을 거부한다.
...
실행별 summary JSON과 stdout·stderr 로그, 두 상태의 불변 조건과 산출물 SHA-256을 묶은 version 1 manifest는 기본적으로 `build/k6/game/`에 생성되며 Git에 커밋하지 않는다. threshold 위반으로 k6가 실패해도 두 시나리오의 결과와 manifest를 먼저 보존한 뒤 실패 코드를 반환한다. 정본 결과는 [키워드 검색 용량 기록](../../../docs/measurements/k6/keyword-search-capacity-2026-08-11.md), [인덱스 판단 기록](../../../docs/measurements/k6/keyword-search-index-decision-2026-08-11.md), [검증 증거 JSON](../../../docs/measurements/k6/evidence/keyword-search-index-comparison-2026-08-11.json)으로 보존한다.
...
이 검증은 두 시나리오의 k6 bundle 해석, 비-200·메타데이터 setup 실패 전파, 고정 검색어, 비교 실행기의 필수 provenance·manifest·로그 계약을 로컬 fake API로 확인한다. 운영 환경에는 부하를 발생시키지 않는다.
