...
| 공통 테스트 작성 규약과 source set 배치 | [src/test/AGENTS.md](src/test/AGENTS.md) |
| PostgreSQL 전용 통합 테스트 규약 | [src/postgresTest/AGENTS.md](src/postgresTest/AGENTS.md) |
...
특정 코드 디렉터리나 source set에 적용되는 규약은 해당 위치의 `AGENTS.md`에 두고, 생산 코드 전반과 협업에 적용되는 공통 규약은 `docs/CONVENTIONS.md`에 둔다. 같은 규칙을 두 위치에 반복하지 않는다.
