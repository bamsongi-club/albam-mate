...
function renderSql(artifact) {
  ...
  const minAgesDesired = values(artifact.minAges.map(row => [row.bggId, row.minAge ?? 'null']), ['bigint', 'integer']);
  ...
  const minAges = `with desired(bgg_id,min_age) as (${minAgesDesired}) update games game set min_age=desired.min_age from desired where game.bgg_id=desired.bgg_id;`;
