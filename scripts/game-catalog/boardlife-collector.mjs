...
async function collectBoardlifeData() {
    ...
    for (const game of BOARDLIFE_NEW_GAMES) {
        ...
        sqlStatements.push(`INSERT INTO games (bgg_id, name, english_name, supported_player_count, tag, estimated_play_time, min_players, max_players, min_play_time_minutes, max_play_time_minutes, complexity, release_year, description, detail_description) VALUES (${game.bgg_id}, '${safeName}', '${safeEn}', '${game.supported_player_count}', '${safeTag}', '${game.estimated_play_time}', ${game.min_players}, ${game.max_players}, ${game.min_play_time_minutes}, ${game.max_play_time_minutes}, ${game.complexity}, ${game.release_year}, '${safeDesc}', '${safeDetail}') ON CONFLICT (bgg_id) DO UPDATE SET name = EXCLUDED.name, description = EXCLUDED.description, detail_description = EXCLUDED.detail_description;`);
    }
