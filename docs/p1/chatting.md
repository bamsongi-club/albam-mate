...
- 프런트의 채팅 POST deadline은 3초다. HTTP 응답 없이 deadline 또는 전송 계층 오류가 발생하면 결과 미확정으로 표시하고 `전송 여부를 확인하지 못했어요. 다시 시도해주세요.` 문구와 `다시 시도` 버튼을 보여준다. 입력은 유지하고 같은 본문 재시도는 같은 `clientMessageId`를 사용하며, 본문을 수정하면 새 식별자를 발급한다. 503·4xx 등 명시적 HTTP 응답은 결과 미확정으로 분류하지 않는다.
...
- `CHAT-02-AC7` HTTP 응답 없이 프런트 deadline이 만료된 전송은 입력과 같은
  `clientMessageId`를 유지한 수동 재시도 상태를 표시하며 자동 재전송하지 않는다.
...
- 메시지 전달 순서의 전역 보장
- 실제 AWS Nginx 분산·장애 처리와 자체 운영 Redis의 HA·TLS·비용 검증
...
- Redis 연결 timeout 1초와 명령·Lua 실행 timeout 2초를 적용하며, 서버 내부 재시도·backoff,
  큐·인메모리 fallback과 Redis 재기동을 기다리는 10초 복구는 P1에서 제공하지 않는다.
