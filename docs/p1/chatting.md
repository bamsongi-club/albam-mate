...
- 채팅 API를 처음 활성화할 때는 ROOM 생성·참가·취소·상태 변경과 자동 상태 보정의
  진행 중인 쓰기를 drain한 뒤 새 ROOM 쓰기를 중지하고, mixed-version rolling 없이
  전진 Flyway 마이그레이션을 한 번 실행한다. 실제 운영 쓰기 중지와 절체에는 사용자·OPS
  승인이 필요하다.
- backfill은 Flyway 트랜잭션의 PostgreSQL `CURRENT_TIMESTAMP`를 하나의 초기화 경계로
  사용하고 아직 채팅방이 없는 모든 기존 ROOM만 삽입한다. 기존 `RECRUITING`·`CLOSED`
  방은 보관 시각을 비워 두고, P1 메시지가 존재할 수 없는 기존 `CANCELED`·`FINISHED`
  방은 같은 기준 시각에 빈 보관을 완료한 것으로 기록한다. 같은 INSERT를 재실행해도
  이미 생성된 행과 보관 값은 변경하지 않는다.
- 마이그레이션 뒤 ROOM과 `CHAT_ROOMS`의 1:1·중복 부재·상태별 보관 값을 검증하고,
  방 생성·최종 상태 연계가 포함된 신버전이 모든 인스턴스에 배치된 뒤에만 ROOM 쓰기와
  채팅 API를 활성화한다. 검증 실패나 구버전 롤백 중에는 ROOM 쓰기를 재개하지 않는다.
  신버전으로 다시 절체해 같은 검증을 통과한 뒤에만 ROOM 쓰기를 재개한다.
...
- `CHAT-01-AC6` `CANCELED` 또는 `FINISHED` 방은 메시지 조회·전송·실시간 구독이
  모두 거절된다.
- `CHAT-01-AC7` 기존 `RECRUITING`·`CLOSED` 방의 backfill 행은 보관 시각이
