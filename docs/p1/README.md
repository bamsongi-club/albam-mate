...
- 기능 상태표상 `CHAT-01`~`CHAT-05`는 모두 `계약 준비 완료`·`구현 완료`·`검증 완료`다. `CHAT-01-AC8`은 현재 첫 운영 배포 계획에서 보류한 조건부 범위다. 이는 관련 ADR까지 모두 검증됐다는 뜻은 아니다.
- 채팅 ADR-0045는 schema-only·local callback 부분은 PR #300·#366으로 확인됐지만, #281의 live backfill·경합·최종 보정·배포 절체를 구현·검증하지 않았으므로 `미검증`으로 유지한다. 따라서 채팅 문서만으로 P1 전체 아카이브 준비 완료를 선언할 수 없고, #281의 재활성화 조건을 남긴다. ADR-0034는 ADR-0049로 대체된 역사적 문서다.
- `운영 배포·실측`은 별도 축이다. [PR #679](https://github.com/bamsongi-club/albam-mate/pull/679)의 route 고정 전용 검증 결과가 있어도 현재 P1 상태표의 `미배포·미측정`을 운영 배포 완료로 바꾸지 않는다. 기존 6개 용량 시나리오와 운영 기준선은 [#650](https://github.com/bamsongi-club/albam-mate/issues/650)에서 별도로 추적한다.
- 이 이슈의 문서 정합화는 아카이브 이동 자체를 수행하지 않는다. P1 완료 결정과 release tag를 확정한 뒤 현재 문서 묶음을 `docs/archive/p1`에 스냅샷하고, 그 시점까지 `docs/p1/`을 현재 정본으로 유지한다.

[P0 문서](../archive/p0/README.md)는 `v0.1.0` 완료 시점 기록으로 동결되어 있으며 P1 구현 작업의 진입점으로 사용하지 않는다. P1도 완료되면 같은 구조로 아카이브한다.

> 문서 관리: 소유자 `밤송이클럽 제품·개발 팀` · 최종 검증일 `2026-08-13` · 폐기 조건 `P1 완료 상태를 확정하고 이 문서를 P1 아카이브로 이동할 때`
