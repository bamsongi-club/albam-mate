...
| `CHAT-04` | 채팅 만료 삭제 잠금 이름·`lockAtMostFor`·실행시간 경고 기준 | #289에서 확정 | 잠금 이름은 `chat-message-retention`이고, 로컬 PostgreSQL 방 50개·메시지 5,000개 대표 배치 126ms를 근거로 경고 1초·`lockAtMostFor` 5초를 [ADR-0049 결정](../adr/chat/0049-chat-message-retention-lock-section-boundary.md#결정), [ADR-0038 결정](../adr/platform/0038-multi-instance-session-and-scheduler-coordination.md#결정)과 [CHAT-04](chatting.md#chat-04-채팅-안전운영)에 반영 |
