...
| `CHAT-03` | [ADR-0052](../adr/platform/0052-local-profile-multi-instance-default.md)에서 확정: 세션 TTL 30분, JSON 직렬화(`SecurityJacksonModules`와 `CurrentUserPrincipal` mixin), `local`·`production` namespace `albam-mate:{env}:session\|ratelimit\|chat:events` | `FND-10`·`CHAT-03` 구현 담당자 | 위임 근거는 [ADR-0038 결정](../adr/platform/0038-multi-instance-session-and-scheduler-coordination.md#결정)과 [ADR-0052 결정](../adr/platform/0052-local-profile-multi-instance-default.md#결정). 선택 결과는 [FND-10](foundation.md#fnd-10-실시간-전달과-재연결-기반), [다중 인스턴스 실행](../ARCHITECTURE.md#다중-인스턴스-실행)과 실행 구성에 반영 |
| `CHAT-04` | 채팅 만료 삭제 잠금 이름·`lockAtMostFor`·실행 상한·질의 시간 상한·구간 수 상한·실행시간 경고 기준 | `CHAT-04` 구현 담당자가 측정 근거로 확정 | 위임 근거는 [ADR-0049 결정](../adr/chat/0049-chat-message-retention-lock-section-boundary.md#결정)과 [ADR-0038 결정](../adr/platform/0038-multi-instance-session-and-scheduler-coordination.md#결정). 선택 결과는 [CHAT-04](chatting.md#chat-04-채팅-안전운영), 실행 구성과 관련 운영 가이드에 반영 |
