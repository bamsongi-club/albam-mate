...
| `CHAT-03` | #360에서 확정: 세션 TTL 30분, JSON 직렬화(`SecurityJacksonModules`와 `CurrentUserPrincipal` mixin), namespace `albam-mate:{env}:session\|ratelimit\|chat:events` | `FND-10`·`CHAT-03` 구현 담당자 | 위임 근거는 [ADR-0038 결정](../adr/platform/0038-multi-instance-session-and-scheduler-coordination.md#결정). 선택 결과는 [FND-10](foundation.md#fnd-10-실시간-전달과-재연결-기반), [다중 인스턴스 실행](../ARCHITECTURE.md#다중-인스턴스-실행)과 실행 구성에 반영 |
