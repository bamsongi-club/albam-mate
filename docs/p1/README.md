...
다음 표는 `CHAT-01`~`CHAT-04`가 `선행 계약 필요`인 이유와 해소 위치를 한곳에 모은다. 실제 상태는 위 상태표만 갱신하며, 세부 선택을 마치면 선택값과 근거를 기능·API·ERD·아키텍처·실행 구성 중 표에 적은 위치에 반영한다. 승인 ADR의 결정 본문은 임의로 수정하지 않으며, 승인 경계를 바꿔야 하면 새 ADR로 대체한다.
