...
| [`CHAT-01`](chatting.md#chat-01-채팅방-생성접근) | 계약 준비 완료 | 부분 구현 ([#279](https://github.com/bamsongi-club/albam-mate/issues/279)) | 부분 검증 ([#279](https://github.com/bamsongi-club/albam-mate/issues/279)) | 미배포·미측정 |
...
다음 표는 채팅 ADR 승인 뒤에 남았던 `CHAT-01`~`CHAT-04`의 선행 계약과 해소 위치를 한곳에 모은다. 실제 상태는 위 상태표만 갱신하며, 세부 선택을 마치면 선택값과 근거를 기능·API·ERD·아키텍처·실행 구성 중 표에 적은 위치에 반영한다. 승인 ADR 링크는 구현 이슈에 결정을 위임한 근거이며 승인된 결정 본문을 수정하라는 뜻이 아니다. 승인 경계를 바꿔야 하면 기존 ADR을 고치지 않고 새 ADR로 대체한다.

| 기능 ID | 착수 전 계약·현재 결과 | 결정 주체 | 위임 근거·반영 위치 |
| --- | --- | --- | --- |
| `CHAT-01` | 확정됨: 최초 활성화 때 ROOM 쓰기를 drain·중지하고 하나의 backfill 경계와 일괄 절체를 사용 | `CHAT-01` 구현 담당자. 실제 운영 쓰기 중지·절체는 사용자·OPS 승인 | 위임 근거는 [ADR-0034 결정](../adr/chat/0034-chat-message-retention-and-deletion.md#결정). 선택 결과는 [CHAT-01](chatting.md#chat-01-채팅방-생성접근), [ERD](../ERD.md#chat_rooms), [채팅 흐름](../ARCHITECTURE.md#채팅-흐름)과 마이그레이션에 반영 |
...
`CHAT-01`과 `CHAT-05`는 제품·API·저장·아키텍처 계약과 필수 ADR에 남은 결정이 없어 `계약 준비 완료`다. 이는 생산 코드가 모두 구현됐거나 `CHAT-01`~`CHAT-03`의 구현 없이 독립적으로 사용자 흐름을 완료할 수 있다는 뜻이 아니다.
