...
이 문서는 P1 필수 범위인 `SEARCH-01`, `SEARCH-02`의 구현 규칙과 완료 기준을 정의하고, 메커니즘 검색과 `SEARCH-03`을 채택 전 후속 후보로 구분해 기록한다. 필수 기능은 현재 미구현이며, 후보는 [P1 명세](../P1-spec.md)의 범위를 변경하기 전에는 P1 구현·완료 계약에 포함하지 않는다.
...
- `SEARCH-01-AC3` `SHORT`, `MEDIUM`, `LONG`이 20분·60분 경계를 포함해 정한 범위대로 동작하고 유효하지 않은 시간은 포함하지 않는다.
