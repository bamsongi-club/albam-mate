...
- `1인 전용`은 `최소 1 ~ 최대 1`에 `인원 정확히 일치`를 더한 조건과 같고 `2인 전용`도 마찬가지다. 그래서 화면은 전용 인원을 하나 고르면 그 경계를 범위 입력에 되비춰 무엇을 고른 상태인지 보여 준다. 이때 되비춘 범위는 표시일 뿐이므로 요청에는 전용 인원 조건만 싣는다.
- 전용 인원을 둘 다 고르면 OR이라 하나의 범위로 나타낼 수 없다. 이때는 범위 입력을 비우고 전용 인원 두 값만 전달한다.
- 범위를 직접 입력하면 전용 인원 선택을 되비추던 상태가 아니므로 선택한 전용 인원을 해제한다. 어느 경우에도 범위 계열과 전용 인원을 함께 담은 요청을 만들지 않는다.
...
- 기존 `playerCount` 조건은 그대로 유지하고 조회 화면은 새 인원 파라미터를 사용한다.
- 조회 화면의 숫자 입력은 마지막 입력 후 0.4초가 지나면 조회하고, 체크박스 선택·해제는 즉시 조회하며 별도 적용 버튼을 두지 않는다.
- 조회 화면의 필터 영역을 닫았다 다시 열어도 현재 입력과 선택을 유지한다.
...
- `SEARCH-01-AC10` 조회 화면의 숫자 입력이 마지막 입력 0.4초 후에 조회하고, 체크박스는 즉시 조회하며 필터 영역을 닫았다 열어도 입력과 선택을 유지한다.
