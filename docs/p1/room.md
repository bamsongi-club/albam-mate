...
| 연결 기능 | [PART-04 선착순 대기열과 자동 승격](#part-04-선착순-대기열과-자동-승격) |
| 구현 전 결정 | 제한 처리 측정의 데이터 규모·ID 수 후보·반복 조건·측정 도구와 로그·메트릭, 안정된 전체 순서의 정렬 키와 cursor 보존·복구 방식, ROOM 전환과 대기열 종료의 트랜잭션·잠금 설계. 한 번당 ID 수와 반복·재시도·실행 주기의 운영 고정값은 측정 결과 뒤 확정하며, 실패 backoff·격리와 조건부 DB 직접 갱신 비교 여부도 기준선 결과 확인 뒤 별도로 결정 |
...
- 제한 ID는 안정된 전체 순서와 cursor 회전으로 선별한다. cursor는 ROOM 처리의 성공 여부와 무관하게 마지막으로 시도한 위치 다음으로 전진하고, 전체 순서의 끝에서는 처음으로 회전한다.
...
- `ROOM-09-AC8` 선별 한도를 반복해서 점유하는 실패 ROOM이 있어도 cursor가 성공 여부와 무관하게 전진·회전하여 같은 관측 집합의 다른 due ROOM이 안정된 전체 순서의 한 순회 안에 처리 기회를 얻는다.
