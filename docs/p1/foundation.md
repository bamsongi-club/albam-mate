...
| --- | --- | --- | --- |
| [FND-09](#fnd-09-검색-성능과-인덱스-검증) | 검색 성능과 인덱스 검증 | `GAME-04`, `ROOM-07`의 검색 의미·대표 쿼리 구현 | P1 검색 성능 근거와 물리 인덱스 확정 |
...
| 구분 | 정본 |
| --- | --- |
| 기능 규칙 | [GAME-04 게임 조건 검색](search.md#game-04-게임-조건-검색), [ROOM-07 방 조건 검색](search.md#room-07-방-조건-검색) |
...
- `FND-09-AC1` `GAME-04`와 `ROOM-07`의 무필터·단일 필터·대표 복합 필터·전체 건수 조회 시나리오가 고정되고 같은 입력으로 반복 실행할 수 있다.
...
- 조건부 후속 범위인 메커니즘·`GAME-05`·인기 랭킹의 성능 검증
