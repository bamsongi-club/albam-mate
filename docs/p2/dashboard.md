...
> **문서 상태: proposed · 정책 값 사용자 확인 완료**
>
> 이 문서는 화면 배치보다 먼저 대시보드가 답할 질문과 판정 근거를 정의한다. 세부 정책 값은 사용자가 확인했지만 P1 아카이브와 P2 활성화 전에는 현재 운영 계약으로 사용하지 않는다.
...
| 경고 등급·이메일·담당자·운영 상태·초기 임계값 | 사용자 확인 완료 | [경고 정책](#경고-정책), P2 활성화 뒤 운영 런북 반영 |
...
| 메트릭·로그 전송 경계 | 방향 확인 완료·프로젝트 ADR 미작성 | [결정 위치와 남은 정본화](monitoring.md#결정-위치와-남은-정본화), P2 활성화 뒤 Platform ADR 이관 |
