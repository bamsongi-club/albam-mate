...
`AI-01`의 후보 조회는 후속 `game.contract` AI-01 후보 조회 port가 소유하고, `DISCOVERY-01`의 `SEARCH-04` tool이나 `game` repository·catalog 직접 접근을 재사용하지 않는 방향으로 경계를 고정한다. 서버가 구조화한 추천 조건은 모두 AND로 적용한 뒤 내부 `RANK-01` 순서로 정렬하며, 공개 `RANK-01` API의 상위 10개 결과나 provider 순서는 사용하지 않는다. 후보 응답 필드는 API·아키텍처 승인 때 확정하고, 기존 `GameQuery` 요약 조회는 이미 확정된 game ID 보강에만 사용한다.
...
- provider payload는 버전이 지정된 instruction·강제 tool schema·기준 시각·서버가 최소화한 사용자 현재 문장·서버가 식별한 누락 필드의 allowlist로 제한한다. 호출 전에 PII·secret을 탐지해 승인된 방식으로 마스킹하고, 안전하게 마스킹할 수 없으면 fail-closed로 거절한다.
- provider 계약은 no-retention·no-training을 명시적으로 보장해야 하며, 정책 버전·URL과 동의 상태를 확인할 수 없거나 provider가 해당 조건을 보장하지 않으면 외부 호출을 하지 않는다.
...
- provider no-retention·no-training 조건의 정책 버전·URL, PII·secret 마스킹·거절 규칙의 최종 승인
...
- confirm은 `Idempotency-Key`와 draft version을 함께 사용한다. 멱등 레코드의 저장·조회·유일성은 최소 `(currentUserId, draft/resource, operation)` 범위로 묶고, 같은 범위의 동일 key만 version 검사보다 먼저 같은 Room을 반환한다. 다른 사용자·draft·operation에서 key를 재사용하면 Room을 반환하지 않고 범위 충돌로 끝내며, 다른 key·오래된 version·만료 초안도 새 Room을 만들지 않는다.
...
- confirmation key의 현재 사용자·대상 draft/resource·operation 범위와 범위 밖 재사용 오류
