...
현재 fixture는 `development-seed` 프로파일의 `draft`입니다. 15개 query와 대표 anchor 3개는 lexical/Sparse/Dense/Hybrid 후보를 비교하는 개발용 입력이며 최종 품질 승인 자료가 아닙니다. Top N membership·snapshot/version pin·index cutover/rollback의 결정은 [ADR-0072](../../adr/game/0072-search-quality-corpus-membership-and-versioning.md)가 소유하고, 이 디렉터리는 그 결정을 실행 가능한 fixture·validator로 고정합니다.

## 데이터 경계

- 검색 품질 평가는 ADR-0066의 초기 **BoardLife Top 1,000 품질 corpus**를 사용합니다.
- membership는 고정된 BoardLife ranking snapshot의 유효 행을 BGG ID로 dedupe하고, 승인 catalog에 매핑한 뒤 unique mapped game을 BoardLife rank·BGG ID 순으로 정렬해 target N을 선택합니다. raw rank cutoff를 mapping보다 먼저 적용하지 않습니다.
- 현재 target N은 1,000이며 5,000·10,000은 별도 reproducible quality evidence가 있을 때만 새 corpus version으로 확장합니다. 전체 BGG 약 17만 건은 이름 검색·구조화 필터·DB index/cache/pagination/부하 검증용이며, 이 fixture의 품질 근거로 혼용하지 않습니다.
- 한국어 데이터 부족을 이유로 membership를 제외·대체하지 않으며, 현재 provisional mapping으로는 quality-ready 승인을 표시하지 않습니다.
...
- `quality-corpus.json`: 고정 ranking snapshot/hash, mapping·dedupe·정렬·target N 규칙과 현재 seed가 참조하는 membership projection
...
- `manifest.index`: corpus version/checksum과 `BUILDING → READY` 또는 `FAILED` 상태, 실패 시 이전 `READY` 유지 규칙
...
이 명령은 profile의 12~15개 범위, 세 cohort, pinned version/snapshot, mapping 뒤 target N membership, hard-filter 호환성, query·corpus checksum을 확인합니다.
