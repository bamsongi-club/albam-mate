...
> **문서 상태: proposed**
>
> 이 문서는 P1에서 조건부 후속으로 예약한 운영 관측 후보를 P2의 `OPS-01`~`OPS-05`로 구체화한다. P1 완료·아카이브와 P2 범위 승인 전에는 현재 운영 계약으로 사용하지 않는다.
...
P2 전체 범위와 공통 운영 흐름은 [P2 공통 명세](../P2-spec.md), 화면·경고·비용·배포 검증 정책은 [운영 대시보드 정책](dashboard.md)이 관리한다. 메트릭·로그 전송 방식은 P2 활성화 뒤 Platform ADR로 이관하며, 이 문서가 프로젝트 ADR이나 구현 증거를 대신하지 않는다.
...
- `OPS-03-AC5` 대표 5xx, Redis 불능, scheduler 실패와 AI 실패 시나리오에서 예상 신호와 복구를 확인한다.
...
| 관측 대상 | [P1 알림](../p1/notification.md), [P1 채팅](../p1/chatting.md), [P1 ROOM·참가](../p1/room.md) |
| 현재 상태 | [P2 기능 상태 정본](README.md#기능별-현재-상태) |
...
메트릭 전송과 로그 전송의 기술 선택은 P2 활성화 뒤 각각 Platform ADR로 이관한다. 현재 프로젝트에는 두 ADR이 없으며, 이 문서만으로 구현·배포 또는 실측이 끝났다고 판정하지 않는다.
