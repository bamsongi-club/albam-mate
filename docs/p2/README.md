...
| [실시간 파티 매칭 명세](matching.md) | `MATCH-01`의 공개 매칭 조건·상태·실시간 흐름, 공통 외부 플랫폼, 투표·채팅·신고·차단과 동시성·실패·복구. 친구 기능은 MVP에서 제외 | 기능 명세 작성 완료·선행 계약 필요 |
...
- 공개 출시 필수 범위: 제한된 FIFO, 게임 ID 목록 스냅샷, 티켓별 공통 외부 플랫폼, 자동 재대기에서는 유지되고 `PAUSED` 후 `다시 찾기`에서는 새로 시작되는 30분 대기 시도, 단일 후보 채팅, `PREPARING` 5분 복구, MATCH 채팅 7일 보존과 개인별 원자료 7일 후 완전 삭제
- 공개 출시 필수 안전 기능: 신고 접수와 차단 목록. 차단한 두 사용자는 이후 새로운 매칭 후보에 함께 포함되지 않으며, 차단 사실을 직접 알리지 않는다. 관리자 화면·운영자 검토·자동 제재·이의제기는 MVP에서 제외한다.
- 명시적 MVP 제외: 친구 관계 생성·관리, 친구 초대 큐, 친구 그룹 매칭과 친구 우선순위. 시간 여유만으로 이 범위를 자동 확장하지 않는다.
