...
P2는 AI 챗봇(게임 탐색 도우미 포함), 게임 의미 기반 검색, 실시간 매칭과 운영 관측·배포 복구를 팀원별 상세 명세로 나눈다. 이 문서는 각 기능 문서의 위치와 계약·생산 코드·자동 검증·배포·실측 상태를 한곳에서 찾는 라우터다.
...
| [운영 관측·배포 복구](monitoring.md) | `OPS-01`~`OPS-06`의 기능별 지표·로그·검증 기준과 P2 CD·앱 rollback 범위 | OPS-01·OPS-02 구현·자동 검증·임시 AWS 실측·철거 완료, OPS-02 외부 API·AI는 해당 기능 미배포로 조건부 제외; OPS-03 구현·자동 검증·임시 AWS 실측·철거 완료, OPS-03 AI·Tool은 해당 기능 미배포로 조건부 제외; OPS-05 통합 검증·dashboard·임시 AWS 실측·철거 완료; OPS-06 코드 구현·자동 검증 완료, 실제 P2 AWS 실행 미검증 |
...
| 게임 의미 기반 검색 | [`SEARCH-04`](search.md#search-04) | 선행 계약 필요 (`#836` T1~T6 승인, public API/ERD와 `#942` index delivery 계약은 남음) | `#836` internal semantic serving core 구현 완료; `#871` public API/UI와 `#942` index delivery 미구현 | `#836` H2·PostgreSQL T1~T6 자동 검증 완료 | 미배포 | 미측정 |
...
| P2 연속 배포·앱 복구 | [`OPS-06`](monitoring.md#ops-06-develop-p2-연속-배포와-앱-복구) | T1~T7 승인 완료 | 앱·인프라 구현 완료 | 정적·H2·PostgreSQL·Docker·Terraform 계약 검증 완료 | 미배포 | 미측정 |
