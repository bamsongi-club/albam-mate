...
| [실시간 파티 매칭 명세](matching.md) | `MATCH-01`의 단일 게임·공통 외부 플랫폼 매칭, 제안·채팅·신고·차단과 동시성·실패·복구·성능 검증 | 기능 명세 작성 완료·선행 계약 필요 |
| [운영 관측](monitoring.md) | `OPS-01`~`OPS-05`의 기능별 지표·로그·검증 기준과 제외 범위 | 정본 승격·선행 ADR 대기 |
...
`MATCH-01`은 P2 정식 범위로 채택한 공개 매칭 기능이다. 별도 초대 기반 파일럿을 두지 않으며, P2 활성화 전에 PRD에 매칭 목적·공통 외부 플랫폼·성공 파티 채팅의 경계를 반영한다. 세부 규칙과 완료 기준은 [MATCH-01 기능 명세](matching.md)가 소유한다.

- 공개 출시 필수 범위: 단일 게임·티켓별 공통 외부 플랫폼, 사용자가 취소할 때까지 시간 경과로 자동 만료되지 않는 `WAITING` 티켓, 제한된 FIFO, 자동 재대기에서는 유지되고 `PAUSED` 후 `다시 찾기`에서는 새로 시작되는 대기 시도, 30초 제안 응답, 성공 파티 채팅, `PREPARING` 5분 복구, MATCH 채팅 7일 보존과 개인별 원자료 7일 후 완전 삭제
- 공개 출시 필수 안전 기능: 신고 접수와 차단 목록. 신고는 신고자·피신고자·고정 사유 카테고리·접수 시각만 7일 보관 후 물리 삭제하며, 자유 입력·첨부·신고 기록 조회를 제공하지 않는다. 신고는 자동 차단으로 이어지지 않고, 차단한 두 사용자는 이후 새로운 매칭 후보에 함께 포함되지 않으며 차단 사실을 직접 알리지 않는다. 관리자 화면·운영자 검토·자동 제재·이의제기는 MVP에서 제외한다.
- 핵심 품질 범위: PostgreSQL 동시성·멱등성 통합 테스트, 연결 중단·재접속 복구 테스트, 후보 탐색·제안 생성 부하 테스트와 성능 개선 전후 측정. 테스트·측정 증거가 없으면 기능 완료로 판정하지 않는다.
- 이번 MVP 제외: 다중 게임 선택·게임 투표, 룰 설명 가능 여부와 링크 공유 담당자 매칭, 5분 대기 후 조건 완화 선택지, `베스트 인원`·`추천 인원` 표시와 점수화, 사용자 간 친구 맺기·친구 목록과 친구 관계 기반 매칭·추천·초대.
...
운영 관측 다섯 기능은 P2 정본 승격이나 [메트릭 ADR-0058](../adr/platform/0058-p2-application-metrics-otlp-host-cloudwatch-agent.md)·[로그 ADR-0059](../adr/platform/0059-p2-structured-stdout-cloudwatch-logs.md) 승인만으로 `계약 준비 완료`가 되지 않는다. 구현할 metric·log 허용 목록, 경고별 query·런북과 배포 설정을 필요한 운영 정본에 반영한 뒤 상태를 바꾼다. `OPS-04`는 AI provider·model·호출 경계가 확정되고 실제 기능이 배포될 때까지 미완료로 유지하지만, 다른 운영 관측 기반의 전달을 막지는 않는다. AI 챗봇은 담당 상세 명세가 등록된 뒤 기능별 선행 계약을 별도로 판정한다. `DISCOVERY-01`과 `SEARCH-04`는 상세 명세 초안을 등록했지만 필요한 API·ERD·아키텍처·ADR·운영 계약이 남아 있어 `선행 계약 필요`다. `MATCH-01`은 핵심 매칭·채팅·신고·차단과 자동 검증·부하·성능 증거를 포함한 상세 명세를 등록했지만, 현재 API·ERD·아키텍처·운영 계약과 필수 ADR에 구현 계약이 반영되지 않아 `선행 계약 필요`다.
