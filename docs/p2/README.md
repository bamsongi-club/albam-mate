...
| 실시간 파티 매칭 | [`MATCH-01`](matching.md#match-01-실시간-파티-매칭) | 계약 준비 완료 | 부분 구현 | 부분 검증: #746 gate manifest는 `MATCH_01_FUNCTIONAL_GATE_ACCEPTED_WITH_T10_DEFERRED`를 기록하고 T1·T5~T7·T12, #775 candidate, #776 response evidence를 기능·정합성 범위의 evidence로 소비; #1000 T11 before/after는 별도 `RESPONSE_COMPARISON_ACCEPTED` evidence로 보존; T2~T4·T8~T9 미검증; T10은 `DEFERRED_BY_ADR_0091`로 성능 축 미검증 | 미배포 | candidate claim·응답 완료 evidence와 T11 before/after 비교는 각 계약 기준에서 `ACCEPTED`; T10 운영 성능 미검증(인프라 수정 후 재검증) |
