...
`MATCH-01`은 P2 정식 범위로 채택한 공개 매칭 기능이다. 별도 초대 기반 파일럿을 두지 않으며, P2 활성화 전에 PRD에 매칭 목적·Board Game Arena 고정 정책·성공 파티 채팅의 경계를 반영한다. 세부 규칙과 완료 기준은 [MATCH-01 기능 명세](matching.md)가 소유한다.

- 공개 출시 필수 범위: 단일 게임·Board Game Arena 고정, 사용자가 취소할 때까지 시간 경과로 자동 만료되지 않는 `WAITING` 매칭 요청, 제한된 FIFO, 자동 재대기와 채팅 준비 실패 복귀에서는 기존 우선순위를 유지하고 `PAUSED` 후 `다시 찾기`에서는 새로 시작되는 대기 시도, 30초 제안 응답, 성공 파티 채팅, `PREPARING` 5분 복구, MATCH 채팅 7일 보존과 개인별 원자료 7일 후 완전 삭제
