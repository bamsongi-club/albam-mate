...
| [`CHAT-08` 채팅 목록 실시간 갱신](chat.md#chat-08-채팅-목록-실시간-갱신) | 사용자 단위 실시간 채널로 목록 화면에 머무는 동안 마지막 메시지·미읽음·방 정렬을 즉시 갱신 | 계약 준비 완료·백엔드 구현·자동 검증 완료(`#918`), 프런트엔드 구현 진행 중(`#919`) |
...
| 채팅 목록 실시간 갱신 | [`CHAT-08`](chat.md#chat-08-채팅-목록-실시간-갱신) | 계약 준비 완료 | 백엔드 사용자 단위 WebSocket·참가자 팬아웃 구현 완료(`#918`), 프런트엔드 구독·재조회 구현 진행 중(`#919`) | 백엔드 H2·PostgreSQL `T1`~`T7` 자동 검증 완료(`#918`) | 미배포 | 미측정 |
