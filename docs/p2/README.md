...
- 공개 출시 필수 범위: 단일 게임·티켓별 공통 외부 플랫폼, 희망 인원 범위 기반 후보 필터링, 제한된 FIFO, 자동 재대기에서는 유지되고 `PAUSED` 후 `다시 찾기`에서는 새로 시작되는 30분 대기 시도, 30초 제안 응답, 성공 파티 채팅, `PREPARING` 5분 복구, MATCH 채팅 7일 보존과 개인별 원자료 7일 후 완전 삭제
- 공개 출시 필수 안전 기능: 신고 접수와 차단 목록. 차단한 두 사용자는 이후 새로운 매칭 후보에 함께 포함되지 않으며, 차단 사실을 직접 알리지 않는다. 관리자 화면·운영자 검토·자동 제재·이의제기는 MVP에서 제외한다.
- 핵심 품질 범위: PostgreSQL 동시성·멱등성 통합 테스트, 연결 중단·재접속 복구 테스트, 후보 탐색·제안 생성 부하 테스트와 성능 개선 전후 측정. 테스트·측정 증거가 없으면 기능 완료로 판정하지 않는다.
- 이번 MVP 제외: 다중 게임 선택·게임 투표, 룰 설명 가능 여부와 링크 공유 담당자 매칭, 5분 대기 후 조건 완화 선택지, `베스트 인원`·`추천 인원` 표시와 점수화.
