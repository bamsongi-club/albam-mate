...
| 실시간 파티 매칭 | [`MATCH-01`](matching.md#match-01-실시간-파티-매칭) | 계약 준비 완료 | 부분 구현, 프런트엔드 연동 완료(`#1041`) | 부분 검증: #746 gate에서 T1·T5~T7·T12, #775 candidate, #776 response evidence만 기능·정합성 범위의 evidence로 소비; T2~T4·T8~T9 미검증; T10은 gate manifest의 `gateDecision`에서 `DEFERRED_BY_ADR_0091`로 별도 기록하고 성능 축은 미검증으로 유지 | 미배포 | candidate claim·응답 완료 evidence는 자체 기준에서 `ACCEPTED`; T10 운영 성능 미검증(인프라 수정 후 재검증) |
