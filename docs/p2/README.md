...
- 공개 출시 필수 범위: 제한된 FIFO, 게임 ID 목록 스냅샷, 티켓별 공통 외부 플랫폼, `PAUSED` 후 사용자의 `다시 찾기`, 단일 후보 채팅, `PREPARING` 5분 복구, MATCH 채팅 7일 보존과 개인별 원자료 7일 후 완전 삭제
- 공개 출시 필수 안전 기능: 신고 진입점과 차단 목록. 차단은 양방향 프로필·개인정보·새 매칭을 막고, 기존 친구 관계가 있으면 조용히 제거하며, 차단 해제 시 친구 관계를 자동 복구하지 않는다.
