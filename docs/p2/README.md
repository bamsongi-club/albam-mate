...
| [운영 관측](monitoring.md) | `OPS-01`~`OPS-05`의 기능별 지표·로그·검증 기준과 제외 범위 | 정본 승격·전송 ADR 승인 |
...
운영 관측 다섯 기능은 P2 정본 승격이나 [메트릭 ADR-0058](../adr/platform/0058-p2-application-metrics-otlp-host-cloudwatch-agent.md)·[로그 ADR-0059](../adr/platform/0059-p2-structured-stdout-cloudwatch-logs.md) 승인만으로 `계약 준비 완료`가 되지 않는다. 구현할 metric·log 허용 목록, 경고별 query·런북과 배포 설정을 필요한 운영 정본에 반영한 뒤 상태를 바꾼다. `OPS-04`는 AI provider·model·호출 경계가 확정되고 실제 기능이 배포될 때까지 미완료로 유지하지만, 다른 운영 관측 기반의 전달을 막지는 않는다. 다른 세 기능은 담당 상세 명세가 등록된 뒤 기능별 선행 계약을 별도로 판정한다.
