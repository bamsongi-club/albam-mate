...
| [운영 관측](monitoring.md) | `OPS-01`~`OPS-05`의 기능별 지표·로그·검증 기준과 제외 범위 | `OPS-01-AC1`~`OPS-01-AC3` 공개 구현·자동 검증 완료, 배포·실측 필요 |
...
| 서비스 생존·연결 | [`OPS-01`](monitoring.md#ops-01-서비스-생존과-연결-상태) | 계약 준비 완료 | `AC1`~`AC3` 구현 완료, `AC4`~`AC7` 미구현 | `AC1`~`AC3` 자동 검증 완료, `AC4`~`AC7` 미검증 | 미배포 | 미측정 |
