...
| 게임 의미 기반 검색 | [`SEARCH-04`](search.md#search-04) | `#836` T1~T6 및 Cloudflare `#942` T1~T5 승인 완료; ERD는 별도 범위 | `#836` internal semantic serving core와 `#942` Cloudflare index delivery 구현 완료; `#871`이 `GET /api/games/search` public read contract와 게임 찾기 화면의 프런트엔드 검색 UI를 구현 완료; [PR #992](https://github.com/bamsongi-club/albam-mate/pull/992)이 Dense + structured/sparse 독립 병렬 후보 생성과 RRF 결합을 구현 완료. ERD·승인 artifact backfill·운영 배포는 보류 | `#836` H2·PostgreSQL T1~T6, `#942` H2 T1/T2/T5·PostgreSQL T3/T4, `#871` H2 T1~T5, [PR #992](https://github.com/bamsongi-club/albam-mate/pull/992) H2·PostgreSQL 자동 검증 완료 | 미배포 | 미측정 |
