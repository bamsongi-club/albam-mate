...
| [실시간 파티 매칭 명세](matching.md) | `MATCH-01`의 매칭 조건·상태·실시간 흐름, 투표·채팅과 동시성·실패·복구 | 기능 명세 초안 작성 완료 |
...
운영 관측 다섯 기능은 P2 정본 승격만으로 `계약 준비 완료`가 되지 않는다. 메트릭·로그 전송 Platform ADR과 필요한 운영 정본을 반영한 뒤 상태를 바꾼다. `OPS-04`는 AI provider·model·호출 경계가 확정되고 실제 기능이 배포될 때까지 미완료로 유지하지만, 다른 운영 관측 기반의 전달을 막지는 않는다. AI 챗봇은 담당 상세 명세가 등록된 뒤 기능별 선행 계약을 별도로 판정한다. `DISCOVERY-01`과 `SEARCH-04`는 상세 명세 초안을 등록했지만 필요한 API·ERD·아키텍처·ADR·운영 계약이 남아 있어 `선행 계약 필요`다. `MATCH-01`은 상세 명세를 등록했지만 현재 API·ERD·아키텍처·운영 계약과 필수 ADR에 구현 계약이 반영되지 않아 `선행 계약 필요`다.
