...
| 실시간 파티 매칭 | [`MATCH-01`](matching.md#match-01-실시간-파티-매칭) | 계약 준비 완료 | 부분 구현 | 부분 검증: #746 gate에서 T1·T5~T7·T12, #775 candidate, #776 response evidence만 `ACCEPTED`; T2~T4·T8~T9 미검증; T10은 현재 #746 gate에서 미완료이며 ADR-0091에 따라 유예 결정됐지만 `DEFERRED_BY_ADR_0091` 기록은 gate manifest 반영 후에만 가능 | 미배포 | candidate claim·응답 완료 evidence는 자체 기준에서 `ACCEPTED`; T10 운영 성능 미검증(인프라 수정 후 재검증) |
