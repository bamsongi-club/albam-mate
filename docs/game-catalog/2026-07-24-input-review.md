...
반복 설명은 게임별 고유 설명으로 검증된 값이 아니라는 품질 한계를 유지한다. 이전 승인은 해당 한계를 인지한 상태에서 현재 입력을 P0 카탈로그로 사용하는 결정이었으며, BGG 상세 데이터 연동 시 교체 검수를 생략한다는 뜻이 아니다.

## 이전 적재 산출물

이전에 승인된 manifest로 변환 도구를 실행해 `quality-report.json` 상태 `ready`와 2,000행 출력을 확인했다. 이후 `games.json`이 팀원이 BGG 데이터를 가공해 만든 자료임을 확인해 현재 manifest 상태를 `pending`으로 전환했다. 아래 체크섬은 이전 실행 결과를 추적하기 위한 기록이며, 필드별 provenance와 이용 근거를 재검증하고 manifest를 다시 승인하기 전에는 해당 산출물을 적재하거나 재사용하지 않는다. 생성 산출물은 저장소에 커밋하지 않는다.
...
- `games.json`의 필드별 provenance와 이용 근거를 재검증하고 manifest를 다시 승인하기 전에는 서비스 카탈로그와 UPSERT SQL을 생성·적재하지 않는다.
