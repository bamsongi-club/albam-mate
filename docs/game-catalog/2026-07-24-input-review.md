...
| --- | --- | --- | ---: | --- |
| 서비스 표시 자료 | `games.json` | Albam Mate 팀원이 BGG 데이터를 P0 표시 필드로 가공해 Slack으로 전달, 2026-07-28 02:32 KST 취득 | 2,000 | `f9adbc203023d1aa20efbfa17b8d52d8db3044ae9525d9c10704255ffcf7e321` |
...
원본 파일과 생성 산출물은 저장소에 복사하지 않는다. BGG CSV는 기준 목록과 외부 식별자 검증에 사용하고, 서비스 표시 필드는 팀원이 BGG 데이터를 가공한 자료를 사용한다. 이 자료를 팀의 독립 원천 데이터로 주장하지 않으며, 세부 출처·이용 범위와 한계는 [출처 manifest](2026-07-24-source-manifest.draft.json)에 기록한다.
...
- `name`이 비어 있는 행은 0건이다.
- 한글이 포함된 833건은 현재 팀 가공 표시명을 유지하되 공식 한국어판 명칭으로 보증하지 않는다.
- 한글이 없는 1,167건은 `name`, `english_name`, `alias`가 같은 BGG 영문명이므로 영문명을 표시 fallback으로 사용한다.
- 공식 근거 없는 음역·번역이나 AI 생성 한국어명은 추가하지 않는다. 공식 국내 명칭 보강은 별도 출처 검수 뒤 `bgg_id` 기준으로 갱신한다.
...
반복 설명은 게임별 고유 설명으로 검증되지 않았다. 이번 승인은 기록된 체크섬 입력의 P0 비상업 적재·로컬 검증에만 적용한다. 공개 상업 운영 권리는 포함하지 않으며, BGG 상세 데이터 연동 시 교체 검수를 생략할 수 없다.

## 승인 적재 산출물

팀원이 BGG 데이터를 가공한 자료라는 실제 출처와 승인 한계를 manifest에 반영한 뒤 변환 도구를 다시 실행했다. `quality-report.json` 상태 `ready`와 2,000행 출력을 확인했으며, 생성 산출물은 저장소에 커밋하지 않는다.
...
| --- | ---: |
| 적재 전 `games` | 0건 |
| 최초 적재 후 `games` / 고유 `bgg_id` | 2,000건 / 2,000건 |
| 한글 포함 표시명 / 영문 fallback / 빈 표시명 | 833건 / 1,167건 / 0건 |
| 최초 적재 후 `bgg_id:id` 매핑 MD5 | `fd8e93e70a43e7f0b6a7b1e008e851cc` |
| 반복 적재 후 `games` / 고유 `bgg_id` | 2,000건 / 2,000건 |
| 반복 적재 후 `bgg_id:id` 매핑 MD5 | `fd8e93e70a43e7f0b6a7b1e008e851cc` |
| `GET /api/games?size=1` | HTTP 200, `totalElements=2000` |

반복 적재 전후 매핑 해시가 같아 `bgg_id`별 내부 `id`가 유지됐고 중복 행이 생기지 않았다.

## 후속 운영

- 추가 번역·AI 처리와 이미지 재호스팅은 별도 이용 근거를 확인한 뒤 진행한다.
...
- 공식 한국어명이나 새 BGG 상세 데이터를 연동할 때 표시명 fallback, 판본 후보 22그룹과 반복 설명 두 경고를 다시 검수한다.
