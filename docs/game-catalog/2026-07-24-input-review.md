...
반복 설명은 게임별 고유 설명으로 검증된 값이 아니라는 품질 한계를 유지한다. 이번 승인은 정확히 기록된 체크섬의 입력을 P0 비상업 적재와 로컬 검증에 사용하는 결정이며, 공개 상업 운영 권리나 BGG 상세 데이터 연동 시 교체 검수를 생략한다는 뜻이 아니다.
...
- 이번 승인은 기록된 체크섬의 P0 비상업 적재·로컬 검증에 한정한다. 공개 상업 운영, 추가 번역·AI 처리와 이미지 재호스팅은 별도 이용 근거를 확인하기 전에는 진행하지 않는다.
