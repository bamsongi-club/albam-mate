...
- `recommended_player_count`는 BGG의 플레이 가능 범위를 옮긴 것으로 보이며, 권장·최적 인원과 같은 의미인지 출처가 확인되지 않았다.
...
2. BGG의 실제 `averageweight`와 권장·최적 인원 취득 근거가 준비될 때까지 현재 `complexity`와 인원 의미를 승인하지 않는다.
