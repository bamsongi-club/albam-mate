...
~~~json
{
  "id": "0e8508a4-5e62-4ac6-bb35-52711a8d42bb",
  "nickname": "알밤"
}
~~~
...
~~~json
{
  "id": "45a73df0-7937-4631-8e0c-f9f0636d1b9e",
  "name": "스플렌더",
...
~~~json
{
  "id": "073e58fe-c902-4257-9bb4-2a168c72c97d",
  "sessionType": "GAME_FOCUSED",
  "title": "토요일 저녁 스플렌더",
  "description": "처음 오신 분도 환영합니다.",
  "game": {"id": "45a73df0-7937-4631-8e0c-f9f0636d1b9e", "name": "스플렌더"},
  ...
  "status": "RECRUITING",
  "joinable": true,
  "host": {"id": "0e8508a4-5e62-4ac6-bb35-52711a8d42bb", "nickname": "알밤"},
  "participants": [
    {"id": "0e8508a4-5e62-4ac6-bb35-52711a8d42bb", "nickname": "알밤"},
    {"id": "c5e8098b-4d6b-4879-9fed-272d85d23f66", "nickname": "밤송이"}
  ]
}
~~~
...
- `participants`는 세션 상세에서만 반환하며 주최자와 현재 `ACTIVE` 참가자를 포함한다.
...
~~~json
{
  ...
  "title": "토요일 저녁 스플렌더",
  "description": "처음 오신 분도 환영합니다.",
  "gameId": "45a73df0-7937-4631-8e0c-f9f0636d1b9e",
  "experienceLevel": "ALL_LEVELS",
  "isRulemasterLed": true,
  "startsAt": "2026-07-25T19:00:00+09:00",
  "place": "홍대입구역 인근 보드게임 카페",
...
- `GAME_FOCUSED`는 유효한 UUID `gameId`가 필수다.
- `PERSON_FOCUSED`의 `gameId`는 선택 사항이며, 게임·태그·카테고리·BGG Weight를 요청으로 받지 않는다.
...
- 주최자만 시작 전 세션을 수정할 수 있다. 세션 유형은 수정할 수 없고, 주최자 외 `ACTIVE` 참가자가 한 명 이상이면 수정할 수 없다.
- `DELETE /api/sessions/{sessionId}`는 삭제가 아니라 `CANCELED` 상태 변경이다.
- `PATCH /api/sessions/{sessionId}/status`는 `{"status":"FINISHED"}`만 받고, 모임 후 `CLOSED` 세션에만 허용한다.

### 4.5 참가·내 모임

- `POST /api/sessions/{sessionId}/participants`는 순차 요청에서 `RECRUITING` 상태·시작 전·중복 참가·남은 모집 자리를 확인하고 참가 관계를 `ACTIVE`로 만든다.
- 참가 후 주최자 외 `ACTIVE` 참가자 수가 `capacity`에 도달하면 `RECRUITING → CLOSED`로 자동 전환한다.
- `DELETE /api/sessions/{sessionId}/participants/me`는 본인만 수행한다. 시작 시각 전 빈자리가 생기면 `CLOSED → RECRUITING`으로 자동 복귀한다.
...
P0 검증은 Swagger 또는 Postman으로 15개 API를 재현하고, 순차 참가·취소에서 모집 인원·참가자 수·상태 전이가 일관되게 계산되는지 확인한다.
