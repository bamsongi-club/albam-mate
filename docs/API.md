...
저장하는 값은 본문의 요청 범위가 아니라 등록 시점에 확정한 교집합이다. 이후 조회 응답의 `minPlayers`·`maxPlayers`와 후보 선별은 이 교집합을 사용하므로, 게임 카탈로그가 나중에 바뀌어도 기존 요청의 판정은 변하지 않는다.
...
> 문서 관리: 소유자 `밤송이클럽 백엔드·프런트엔드 팀` · 최종 검증일 `2026-08-15` · 폐기 조건 `HTTP·WebSocket 계약이 승인된 다른 정본에서 생성되고 이 문서가 그 정본으로 대체될 때`
