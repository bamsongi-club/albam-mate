...
- 근거:
    ...
    - 재시도: `RoomWaitlistRegistrationCoordinator`가 ROOM 충돌과 정확한 순번 UNIQUE 충돌을 최초 포함 총 3회의 단일 예산으로 분류한다. 단위 테스트는 대상 제약 식별자 분류·공유 예산·최종 409/500과 허용된 로그 필드를 고정하고, `RoomWaitlistConcurrencyPostgresTest`의 T5는 실제 순번 UNIQUE 충돌의 독립 세 번 시도에서 ROOM·대기행이 롤백됨을 검증한다.
...
- 미검증:
    - PK·FK·CHECK·그 밖의 UNIQUE, 교착·직렬화 실패와 분류할 수 없는 DB 오류의 공통 500·정제된 `ERROR` 로그 경계가 결정 전체를 충족하지 않는다. 현재 `RoomWaitlistRegistrationCoordinator`는 `DataIntegrityViolationException`만 ROOM ID와 함께 처리하고, 교착·직렬화 같은 예외는 전역 처리기로 넘어가 ROOM ID 없이 기록된다. 원인과 ROOM을 추적하는 요청당 한 번의 로그 및 그 테스트가 필요하다.
