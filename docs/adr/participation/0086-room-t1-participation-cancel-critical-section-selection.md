...
| V2. 첫 WAITING 선택·조건부 승격 왕복 축소 | 현재 FIFO 후보 선택과 허용된 `WAITING → PROMOTED` 전이를 더 적은 데이터베이스 왕복으로 처리할 수 있다. | 한 SQL 또는 Repository 계약이 FIFO 선택, 조건부 전이 결과와 다음 후보 재시도에 필요한 정보를 안전하게 반환해야 하며 정확한 query shape는 아직 확정되지 않았다. | 비교 후보, 미선택 |
...
1. 측정 직전에 하나의 공통 source base를 고정하고 V0·V1·V2의 code SHA와 변경 경로를 분리한다. PR #947이 그 base와 다르면 후보 A를 같은 base로 rebase하거나 재구성한 뒤 측정한다.
...
정확한 성능 임계값과 동률 처리 규칙은 아직 승인되지 않았다. 이 항목과 후보 B의 정확한 query shape가 확정되고, 별도 승인된 AWS 실행과 #781의 유효 결과가 생기기 전에는 이 ADR을 `승인됨`으로 바꾸지 않는다.
...
- 감수할 비용·위험:
    ...
    - 후보 B의 query shape를 확정하는 과정에서 Repository 반환 계약과 PostgreSQL 전용 검증이 추가될 수 있다.
- 후속 작업:
    - #781에서 후보 B 구현 경계와 사전 정량 기준을 승인하고 V0·V1·V2의 정합성·성능·DB 비용 evidence를 만든다.
...
- 미검증:
    - 측정용 공통 source base와 V0·V1·V2 code SHA
    - 후보 B 구현과 V0·V1·V2의 단위·PostgreSQL correctness 결과
