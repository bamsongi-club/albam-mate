...
V0 현행 persistence shape를 유지한다. 후보 A(V1)와 후보 B(V2)는 이번 production 선택에서 채택하지 않으며, Draft PR #947·#960을 병합하지 않는다.
...
| variant | 원본 SHA | fixture-only derived SHA | 성공 p95 (ms) | p95 중앙값 | 성공 / 허용 409 합계 | T5 판단 |
| --- | --- | --- | --- | ---: | ---: | --- |
| V0 | `d3ea2a9` | `43b07457` | 613.387 / 233.240 / 190.070 | 233.240 | 48 / 72 | 기준선 유지 |
...
V1과 V2는 모두 세 run 중 두 run에서 V0보다 낮은 p95를 보였지만, 사전 승인된 핵심 기준인 p95 중앙값 최소 20% 개선을 충족하지 못했다. V1은 성공 건수와 허용 409도 V0보다 불리했다. V2는 성공·허용 409 합계와 업무 완료율 guardrail에 문제는 없었지만, p95 중앙값은 크게 악화됐다. 따라서 추가 조건을 실행하지 않고 V0을 유지한다는 T5 fallback을 적용한다.

## 결과

- 얻는 것:
    - 후보 구현과 production 선택을 분리하고, 사전 승인된 T5로 V0 유지 근거를 Participation 정본에 남긴다.
    - correctness 실패나 불완전한 artifact를 빠른 후보로 오인하지 않고, 유효한 승자가 없을 때의 fallback을 실제 결과로 검증했다.
    - ADR-0046의 저장 불변식과 ADR-0083의 retry 전략을 V0의 공통 전제로 유지한다.
- 감수할 비용·위험:
    - V1의 ROOM write 축소와 V2의 PostgreSQL CTE 왕복 축소는 이번 T1 hot c8 결과에서 production 이득을 입증하지 못했다.
    - V0의 첫 run p95 변동이 커, 더 넓은 조건을 새로 실행하려면 별도 이슈와 사전 판정 기준이 필요하다.
- 후속 작업:
    - V1·V2 Draft PR은 merge하지 않은 채 결과와 재검토 조건을 보존한다.
    - 새 후보·새 부하 조건 또는 attribution 도구를 도입하려면 #781 범위를 재사용하지 않고 별도 이슈에서 계약을 승인한다.

## 적용·호환·rollback

- 적용: V0 persistence shape를 유지하므로 이번 결정에 따른 runtime 코드, API, DB schema, 배포 구성 변경은 없다.
- 호환: 어떤 후보를 선택해도 외부 API·오류 envelope, DB schema, FIFO 정책, T2 등록, retry/backoff와 전역 잠금 전략은 바꾸지 않는다.
- rollback: 이번 결정은 V1·V2를 반영하지 않으므로 별도 production rollback이 없다. 이후 후보를 반영했다가 회귀가 확인되면 해당 변경을 되돌려 V0 persistence shape로 복구한다.
...
- 근거:
    ...
    - 계약:
        - #781의 최신 승인 T1~T5는 V0·V1·V2의 독립 변경 귀속, correctness hard gate, T1 hot c8 세 번씩의 측정과 보수적 V0 fallback을 요구한다.
    - 테스트:
    ...
    - AWS 측정:
        - V0·V1·V2 각각 세 run, 총 아홉 run의 manifest, raw artifact와 digest, k6 exit code, before/after diagnosis, final result, resource signal을 확인했다.
        - 모든 run은 `VALID/PASS`였고, 위 표의 p95 중앙값과 guardrail로 T5를 적용해 V0 유지 결론을 확인했다.
