...
> 2026-08-06 승인된 ADR-0051이 이 문서의 후속 작업에 적힌 ALB·ASG 경로를 App1 Nginx·고정 EC2 경로로 바꿨다. ADR-0052는 본문의 `local-multi` 실행 명칭을 현재 다중 인스턴스 `local`로 통합했다. HTTP 저장·WebSocket 수신 결정과 재연결 계약은 그대로 유효하다.
