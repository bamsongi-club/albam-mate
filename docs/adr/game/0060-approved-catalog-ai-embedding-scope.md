...
1. `approved: true`, `testOnly: false`인 하나의 catalog release에 대해 manifest가 허용한 필드와 `approvedProcessingScopes` 범위에서만 AI 입력·embedding 생성을 허용한다.
...
- 얻는 것: 승인된 dataset으로 SEARCH-04의 `search_text`·embedding 평가를 진행할 수 있고, release·필드·모델 변경을 재현 가능한 index version으로 추적할 수 있다.
...
- 보류 이유: 이번 결정은 데이터 이용 범위의 승인이지 검색 기술·API·저장 구조의 결정이 아니다.
