...
또한 정규화한 단독 게임명에 유일한 카탈로그 정식명 매치가 있을 때에도 provider로 보내면 비용·quota를 소비하고, 모델에게 게임 검색·식별 권한을 주지 않는 ADR-0074의 경계와도 맞지 않는다. 이 결정은 provider를 확장하지 않고 서버의 `game.contract` 조회로 그 경우를 처리한다.
...
- 유일한 정식 게임명은 provider·AI quota·비용 예약을 거치지 않되, 모델에 게임 검색 권한을 주지 않는다.
...
- `POST /api/assistant/recommendations`는 기존 인증·CSRF·feature gate·외부 처리 동의 gate를 통과한 뒤, provider 호출 전에 단독 입력의 정확 게임명 조회를 수행한다.
- 정확 매치는 입력과 `Game.name`에 Unicode NFKC, 앞뒤 공백 제거, 연속 공백 하나로 축약, `Locale.ROOT` 대소문자 정규화를 적용한 값이 유일하게 같은 경우다. 문장 부호 제거, 부분 일치, 별칭·영문명·BGG ID, 기본판과 확장판의 자동 통합은 하지 않는다.
...
- 얻는 것:
  ...
  - 유일한 정식 게임명을 빠르게 카탈로그 후보로 연결하고 외부 provider 호출을 피한다.
- 감수할 비용·위험:
  - 앱 전체 월 최대 50회의 실제 provider 호출 제한은 40명·월 150회 사용자 quota보다 먼저 작동할 수 있다.
  - 정확 매치는 별칭과 문장 안 언급을 해결하지 않으며, 0건·복수건은 일반 추천 흐름으로 돌아간다.
- 후속 작업:
...
- 지금 하지 않는 것: token 가격 기반 hard cap, 별칭·영문명·문장 내 게임명 추출, provider 검색 권한, 추천 이력·부분 초안의 서버 영속화.
...
- 미검증:
  ...
  - 유일·0·복수 정확 게임명과 provider·quota·비용 예약 0건 경로
