...
### 정확 게임명 후보와 확인 UI의 경계

`카탄 모임 만들어줘`처럼 게임명과 모임 의도가 한 문장에 함께 있어도 이 ADR이 확정하는 결과는 `카탄`의 유일한 `gameId`를 가진 후보 한 건까지다. 클라이언트는 이 후보를 확인 카드·모달로 표시하고, 사용자가 최종 조건을 명시적으로 확인한 뒤에만 AI-03의 초안·Room 생성 경계로 이동한다. 정확 게임명 resolver는 provider 내부 tool이나 의미 기반 검색을 호출하지 않으며, 후보·초안·Room·ChatRoom을 직접 생성하지 않는다.

따라서 providerless 직접 조회의 quota·비용 0건 규칙과 카드·모달의 화면 표현, 초안·Room의 원자성은 서로 다른 책임이다. 전자는 이 ADR과 `game.contract`가 소유하고, 후자는 [AI 기능 명세](../../p2/assistant.md)·[API](../../API.md)·ADR-0075가 소유한다.
