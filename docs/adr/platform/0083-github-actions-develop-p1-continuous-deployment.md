...
- deploy role은 게시된 image의 ECR 읽기와 P1 대상 SSM 명령·결과 조회·배포 기록 권한만 가진다. ECR image 쓰기, Terraform apply, IAM 변경, 비밀값 조회 범위를 배포 권한에 섞지 않는다.
...
- 후속 작업:
    ...
    - P1 실제 배포에서 SHA/digest, migrator 결과, App2/App1 health, smoke, rollback 결과를 보존한다.
...
- 미검증:
    ...
    - one-shot migrator와 App1·App2 Flyway 비활성화, expand/contract 검증
