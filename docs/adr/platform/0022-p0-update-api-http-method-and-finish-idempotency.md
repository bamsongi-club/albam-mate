...
- 근거:
    - 테스트:
        - PR #98의 `RoomStatusChangeExecutorIntegrationTest`가 자동 정합화 상태의 커밋을 검증했다.
        - PR #98의 `RoomStatusChangeExecutorIntegrationTest`가 자동 정합화 상태의 버전 증가를 검증했다.
        - PR #98의 `RoomStatusChangeExecutorIntegrationTest`가 기존 `FINISHED`의 상태 무변경을 검증했다.
        - PR #98의 `RoomStatusChangeExecutorIntegrationTest`가 기존 `FINISHED`의 버전 무변경을 검증했다.
        - PR #98의 `RoomStatusChangeExecutorIntegrationTest`가 기존 `FINISHED`의 갱신 시각 무변경을 검증했다.
