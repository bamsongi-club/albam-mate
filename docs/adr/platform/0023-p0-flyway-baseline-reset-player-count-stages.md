...
- 상태: 검증됨
- 근거: 새 V1~V3를 적용한 `PostgresSchemaValidationTest`가 PostgreSQL 18 Testcontainers에서 가능·추천·최적 인원 컬럼의 단계별 존재와 NULL 정책, 기존 값 보존을 확인한다. `./gradlew build --no-daemon`, `./gradlew postgresTest --no-daemon --stacktrace`, `node scripts/check-doc-links.mjs`가 통과했다.
