...
공개 목록과 내 모임 목록은 이 유효 상태를 필터·정렬·content/count·페이지·응답 `status`·`chatAvailable`·참가 및 대기 가능 여부에 일관되게 사용한다. 페이지를 조회한 뒤 DTO의 일부 상태만 바꾸는 방식은 사용하지 않는다. 유효 상태 판정에 필요한 `requestTime`은 조회 조건에 전달하며, 데이터베이스의 실행 시각이나 행마다 다른 현재 시각에 의존하지 않는다.
...
- 얻는 것:
    ...
    - 공개 목록의 필터·count·페이지와 응답 상태가 같은 유효 상태 기준을 사용한다.
...
- 후속 작업:
    - #557에서 유효 상태를 조회 조건과 응답 조립에 적용하고, 목록·내 모임 GET에 DML·이벤트·전역 backlog 적재가 없음을 검증한다.
