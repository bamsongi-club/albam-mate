...
| [알림 Outbox 운영 런북](NOTIFICATION_OPERATIONS.md#현재-운영-파라미터-정본) | 알림 relay 수치·측정, 실패 복구·폐기와 보존 데이터 정리 | P1 계약 | 현재 운영 파라미터와 실행·판정 절차를 소유한다. 실행 가능 여부는 [P1 기능 상태 정본의 `NOTI-01`](../p1/README.md#기능별-현재-상태), 결정 근거는 [ADR-0030](../adr/notification/0030-postgresql-notification-relay-processing-recovery.md), 저장 계약은 [ERD의 P1 알림 저장 계약](../ERD.md#p1-알림-저장-계약)을 따른다. |
