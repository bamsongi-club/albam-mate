...
| [P2 운영 관측 런북](MONITORING_OPERATIONS.md) | metric·log 허용 목록, alarm 대응, 전체 스택 계획 종료·재기동과 증거 계약 | 계약 정본·구현 전 | 구현자가 따라야 할 inventory·상태 전이·IAM·실패 복구를 소유한다. 기능 완료 기준은 [P2 운영 관측](../p2/monitoring.md), 화면·등급 정책은 [운영 대시보드](../p2/dashboard.md), 실행 명령은 구현 뒤 [COMMANDS](../COMMANDS.md#운영-compose)에 등록한다. |
...
`사용 가능`, `구현·배포 전 기준`과 `계약 정본·구현 전`은 현재 실행 상태를 나타낸다. `승인·부분 검증`은 임시 AWS 배포와 일부 부하 실행 증거가 있지만 독립 재현·복구·교차 인스턴스·지속 용량 경계는 확인하지 않았음을 뜻한다. `P1 계약`은 도입 단계만 나타내며 현재 생산 코드·검증·배포 여부는 연결된 P1 기능 종료 상태로 판정한다.
