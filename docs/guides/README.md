...
| [P1 AWS 자체 운영 인프라 제안 실행안](AWS_MULTI_INSTANCE_INFRASTRUCTURE.md) | 네 EC2를 `t4g.micro`로 시작해 역할별 병목을 측정하고 단계적으로 확장하는 Terraform 배포·검증 절차 | 제안 실행안 | 대화에서 직접 제시된 범위와 추가 설계를 구분하고 실행 순서와 검증 항목을 설명한다. 기술 선택과 기존 기준 대체 여부는 [제안 ADR-0051](../adr/platform/0051-p1-self-managed-aws-infrastructure.md)이 소유한다. |
