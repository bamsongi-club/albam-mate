...
| [develop P2 자동 CD 가이드](CD_DEPLOYMENT.md) | `develop` 성공 CI SHA·이미지·migrator·App2→App1 롤아웃과 앱 rollback 경계 | 앱 코드·자동 검증 완료, target binding·P2 AWS 실행 보류 | 기존 4노드 Terraform target의 stable CD lifecycle을 승인한 뒤 실행할 P2 처리·증거 경계를 설명한다. 구현 계약은 [#961](https://github.com/bamsongi-club/albam-mate/issues/961)의 승인된 T1~T7이며, 실제 P2 AWS 실행·복구 증거는 아직 없다. |
