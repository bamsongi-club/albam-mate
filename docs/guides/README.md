...
| [develop P1 자동 CD 가이드](CD_DEPLOYMENT.md) | `develop` 병합 뒤 동일 SHA의 CI·이미지·migrator·App2→App1 롤아웃과 앱 rollback 경계 | 승인·구현 전 | 실행·실패 처리·증거 경계를 설명한다. 기술 선택은 [ADR-0083](../adr/platform/0083-github-actions-develop-p1-continuous-deployment.md)이 소유하며, workflow·OIDC·migrator·실제 AWS 실행은 아직 구현·검증되지 않았다. |
