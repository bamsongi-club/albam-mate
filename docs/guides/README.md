...
`사용 가능`, `구현·배포 전 기준`과 `계약 정본·구현 전`은 현재 실행 상태를 나타낸다. `계약 정본·OPS-01 구현·AWS 실측`은 `OPS-01`의 생산 코드·자동 검증·임시 AWS 배포·측정·철거를 마쳤다는 뜻이며 상시 운영 배포나 다른 `OPS` 기능 완료를 뜻하지 않는다. `승인·부분 검증`은 임시 AWS 배포와 일부 부하 실행 증거가 있지만 독립 재현·복구·교차 인스턴스·지속 용량 경계는 확인하지 않았음을 뜻한다. `P1 계약`은 도입 단계만 나타내며 현재 생산 코드·검증·배포 여부는 연결된 P1 기능 종료 상태로 판정한다.
