...
| [P2 운영 관측 런북](MONITORING_OPERATIONS.md) | metric·log 허용 목록, alarm 대응, 전체 스택 계획 종료·재기동과 증거 계약 | 계약 정본·부분 구현·AWS 실측 | inventory·상태 전이·IAM·실패 복구와 `OPS-01-AC4`~`AC7` 실측 경계를 소유한다. 기능 완료 기준은 [P2 운영 관측](../p2/monitoring.md), 화면·등급 정책은 [운영 대시보드](../p2/dashboard.md), 반복 실행 명령은 [COMMANDS](../COMMANDS.md#운영-compose)를 따른다. |
...
`사용 가능`, `구현·배포 전 기준`과 `계약 정본·구현 전`은 현재 실행 상태를 나타낸다. `계약 정본·부분 구현·AWS 실측`은 연결 기능 중 명시한 AC만 구현·검증·임시 배포·측정을 마쳤고 상위 기능 전체 완료는 아니라는 뜻이다. `승인·부분 검증`은 임시 AWS 배포와 일부 부하 실행 증거가 있지만 독립 재현·복구·교차 인스턴스·지속 용량 경계는 확인하지 않았음을 뜻한다. `P1 계약`은 도입 단계만 나타내며 현재 생산 코드·검증·배포 여부는 연결된 P1 기능 종료 상태로 판정한다.
...
> 문서 관리: 소유자 `밤송이클럽 개발·운영 팀` · 최종 검증일 `2026-08-14` · 폐기 조건 `최초 설정·운영 가이드의 대표 인덱스가 승인된 다른 문서로 대체될 때`
