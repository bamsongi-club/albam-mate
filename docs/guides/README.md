...
| [P1 AWS 저비용 4 EC2 인프라 실행안](AWS_MULTI_INSTANCE_INFRASTRUCTURE.md) | App1 Nginx 단일 진입점과 네 `t4g.micro` EC2를 Terraform으로 만들고 역할별 병목을 측정하는 절차 | 승인·배포 전 | 승인된 P1 방향, Terraform·Ansible 1차 구현과 아직 미구현인 애플리케이션·운영 경계를 구분한다. 기술 선택과 기존 기준의 부분 대체 범위는 [ADR-0051](../adr/platform/0051-p1-self-managed-aws-infrastructure.md)이 소유한다. |
...
`사용 가능`과 `구현·배포 전 기준`은 현재 실행 상태를 나타낸다. `승인·배포 전`은 기술 결정과 실행 순서는 채택됐지만 실제 AWS 배포·복구·부하 검증은 끝나지 않았음을 뜻한다. `P1 계약`은 도입 단계만 나타내며 현재 생산 코드·검증·배포 여부는 연결된 P1 기능 상태 정본으로 판정한다.
