...
| 외부 fixture PostgreSQL 성능 | `postgresPerformanceTest` | 저장소 밖 17만 행 fixture의 대표 검색 조합·실행 계획·응답 시간 |
...
`GameMetadataSearchPerformancePostgresTest`는 저장소 밖 fixture와 순위 CSV를 요구하므로 기본 `postgresTest`에서 제외한다. 네 입력 경로를 준비한 뒤에는 [게임 카탈로그 적재 가이드](GAME_CATALOG_IMPORT.md#17만-행-성능-fixture-계약)의 명령으로 `postgresPerformanceTest`를 별도로 실행한다.
