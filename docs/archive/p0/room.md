...
- 필터를 생략하면 두 유형의 공개 방 전체를 조회하고, `type`·`gameId`·`keyword`가 전달되면 선택 조건을 AND로 적용한다.
- 게임 중심·사람 중심 탐색은 같은 방 데이터를 사용하며 화면의 강조점만 다를 수 있다.
- 빈 문자열과 공백 `keyword`는 검색 조건 없음으로 처리하고, 제목 부분 일치는 대소문자를 구분하지 않는다.
- 검색 조건, 공개 대상, 정렬, 페이지네이션과 응답 필드는 API 계약을 따른다.

### 완료 기준

- `ROOM-01-AC1` 게임 중심 조회는 선택한 게임에 해당하는 공개 방만 반환한다.
- `ROOM-01-AC2` 사람 중심 조회는 검색어가 있으면 제목 부분 일치 결과를 반환한다.
- `ROOM-01-AC3` 상태 정합화가 끝난 현재 공개 대상만 목록에 포함하며 `joinable`은 요청자 기준으로 계산한다.
- `ROOM-01-AC4` 비로그인 사용자도 방 목록을 조회할 수 있다.
- `ROOM-01-AC5` 필터 없는 조회는 두 유형의 `RECRUITING`, `CLOSED` 공개 방을 반환한다.
- `ROOM-01-AC6` `type`, `gameId`, `keyword`는 독립적인 선택 필터이며 각 단독 필터와 조합 필터는 전달된 조건을 모두 만족하는 공개 방만 반환한다.
- `ROOM-01-AC7` 공개 목록은 시작 시각, 방 ID 오름차순과 페이지네이션을 유지하며 개인정보를 포함하지 않는다.
...
- `ROOM-02-AC1` 비로그인 사용자와 방 관계가 없는 사용자는 공개 범위의 상세만 조회한다.
- `ROOM-02-AC2` 주최자와 현재 `ACTIVE` 참가자는 관계자 범위의 상세를 조회한다.
- `ROOM-02-AC3` 관계자 응답만으로 요청자가 주최자인지 참가자인지 구분할 수 있어 방 관리와 참가 취소 중 어떤 동작을 제공할지 결정할 수 있다.
- `ROOM-02-AC4` 최종 상태 방의 조회 권한과 존재 여부 은닉이 공통 권한 규칙과 일치한다.
- `ROOM-02-AC5` 방 유형에 따라 게임 정보 또는 `게임 미정`이 일관되게 표시된다.
- `ROOM-02-AC6` 관계자 응답은 API의 캐시 방지 헤더를 반환하며, 참가 취소·로그아웃 뒤 같은 URL에서 이전 정확한 장소와 참가자 정보가 재사용되지 않는다.
...
- `ROOM-03-AC1` 유효한 요청으로 생성한 방이 공통 정원·최초 상태·시간 규칙을 만족한다.
- `ROOM-03-AC2` 방 유형별 게임 선택 규칙을 위반한 요청은 API 계약의 오류로 거절한다.
- `ROOM-03-AC3` 생성자는 주최자로 기록되고 방 관계자 범위의 생성 결과를 받는다.
- `ROOM-03-AC4` 생성된 방의 지역은 홍대로 저장되며 내 모임의 개설 목록에서 확인할 수 있다.
...
- `ROOM-04-AC1` 수정 가능한 방의 허용 필드만 변경되고 나머지 값은 유지된다.
- `ROOM-04-AC2` 주최자가 아니거나 수정 가능 상태·시간·참가자 조건을 충족하지 않으면 계약된 오류를 반환한다.
- `ROOM-04-AC3` 방 유형, 지역 또는 상태를 포함한 요청은 API 계약에 따라 거절한다.
- `ROOM-04-AC4` 정원 변경과 참가 요청이 겹쳐도 저장 불변식이 유지된다.
...
- 방 목록·상세·내 모임 조회와 상태에 의존하는 명령은 각 조회·명령 로직 전에 현재 방 상태를 정합화한다.
...
- `ROOM-06-AC1` 시작 경계를 지난 방은 상태에 의존하는 조회와 명령에서 현재 상태로 판정된다.
- `ROOM-06-AC2` 자동 종료 경계를 지난 방은 상태 정합화 후 종료 상태로 판정된다.
- `ROOM-06-AC3` 목록과 내 모임은 상태 보정이 반영된 뒤 필터와 페이지를 계산한다.
- `ROOM-06-AC4` 동시 보정이 다른 최종 상태를 덮어쓰지 않으며, 재시도 실패는 API 계약의 오류로 귀결된다.
- `ROOM-06-AC5` 경계 시각과 동시 요청 검증은 PostgreSQL 기반 통합 테스트로 통과한다.
