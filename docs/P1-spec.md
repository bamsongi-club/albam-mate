...
- 조건부 후속 범위는 `OPS-01`, `SEC-01`, `OPS-02`를 예약하되, 범위 채택 전에는 완료 기준 ID를 부여하지 않는다.
...
- 인기 게임 랭킹은 `GAME_FOCUSED`이고 `CANCELED`가 아닌 내부 방만으로 전체·앞으로 7일 집계를 만들고, 한 요청의 고정 기준 시각, 안정 정렬과 상위 10개 상한, 공개 필드 경계를 지킨다.
- 조건부 후속 범위인 `OPS-01`, `SEC-01`, `OPS-02`의 미구현은 P1 필수 범위 완료를 막지 않는다.
