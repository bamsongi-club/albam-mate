...
`ROOM-10`은 P1 필수 범위에 그대로 두되, 그 안의 비관적 락 비교와 최종 잠금 전략 ADR은 후속으로 이관한다. P1은 현행 낙관적 락의 기준선 측정과 저장 불변식 검증까지로 완료를 판정한다. 기준선은 경합을 강제한 fixture 결과라 잠금 전략 교체 여부를 단독으로 판단할 입력이 되지 못하고, 그 판단에 필요한 실사용 동시성 계측은 배포 후에 확보하기 때문이다. 이 결정과 재검토 조건은 [#495](https://github.com/bamsongi-club/albam-mate/issues/495)에 기록했다.
...
- 다른 대기자의 신원·전체 대기자 목록 공개와 대기 순번 변경 알림
...
| 참가 취소와 함께 첫 대기자 자동 승격 성공 | 실제 승격 사용자 | 참가 확정 확인 |
...
- 참가 취소와 첫 대기자 자동 승격이 같은 트랜잭션에서 커밋되면 실제 승격 사용자에게만 승격 알림 이벤트를 기록한다. 이때 실제 빈자리가 없으므로 주최자용 참가 취소 알림 이벤트는 만들지 않는다. 활성 대기자가 없어 빈자리가 남을 때만 기존 참가 취소 알림 이벤트를 기록한다.
...
- 네 가지 알림 원인마다 커밋 시점의 정확한 수신자에게 알림이 한 번 생성되고 목록, 미확인 개수와 단건·일괄 읽음이 일관된다.
