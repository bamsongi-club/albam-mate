...
| [소셜 로그인](p1/auth.md) | Google·Naver·Kakao 로그인과 기존 계정의 명시적 연결 |
...
- 인증은 상위 `AUTH-05`와 구현 단위 `AUTH-05a`~`AUTH-05d`, 검색은 `SEARCH-01`, `SEARCH-02`, 방은 `ROOM-08`~`ROOM-10`, 참가 대기열은 `PART-04`를 사용한다.
...
- 조건부 후속 범위는 `SEARCH-03`, `RANK-01`, `OPS-01`, `SEC-01`, `OPS-02`를 예약하되, 범위 채택 전에는 완료 기준 ID를 부여하지 않는다.
...
P1 필수 구현은 다음 일곱 가지 흐름을 처음부터 끝까지 연결한다.

1. Google·Naver·Kakao로 처음 로그인하거나 재로그인하고, 같은 이메일의 기존 계정은 로그인 뒤 명시적으로 연결하는 흐름
2. 조건에 맞는 게임과 방을 찾고 기존 P0 흐름으로 방 상세·참가까지 이동하는 흐름
3. 시작 전 정원이 찬 방에 대기하고 참가 취소로 생긴 빈자리에 첫 대기자가 자동 승격되는 흐름
4. 방·참가 변경 뒤 정확한 수신자가 앱 내 알림을 확인하고 읽음 처리하는 흐름
5. 방 참가 직후 채팅 이력을 조회하고 메시지를 주고받으며 재접속 뒤 누락분을 복구하는 흐름
6. 시간 경계를 지난 ROOM을 제한된 단위로 자동 전환하고 ROOM별 실패를 격리·재처리하는 흐름
7. 참가·대기·자동 승격·상태 자동 전환의 동시 요청을 재현해 정합성과 낙관적 락의 비용을 측정하는 흐름
...
| --- | --- | --- | --- |
| 인증 | `AUTH-05` | Google·Naver·Kakao 로그인, provider subject 기반 사용자 식별과 기존 계정의 명시적 연결 | [소셜 로그인](p1/auth.md#auth-05-소셜-로그인계정-연결) |
| 게임 탐색 | `SEARCH-01` | 약 2,000개 게임의 인원·시간·난이도 데이터를 검색 가능한 값으로 정규화하고 복합 필터 제공 | [게임 조건 검색](p1/search.md#search-01-게임-조건-검색) |
...
| 영역 | 기능 ID | 후속 범위 |
| --- | --- | --- |
| 게임 탐색 확장 | `SEARCH-01` 확장 후보 | 검수된 약 10~20개 메커니즘의 단일·다중 선택 검색 |
| 해 본 게임 | `SEARCH-03` 후보 | 사용자별 해 본 게임 등록·취소와 포함·제외 검색 |
...
- 운영 Redis 제품·HA·TLS·접근 제어·비밀 주입·비용 확정과 실제 AWS ALB·ASG scale-out·draining 검증
...
> `AUTH-05`는 #328에서 팀 승인받기 위한 P1 필수 범위 제안이며, 문서·ADR 승인 전에는 구현하지 않는다.

세부 기능 규칙과 완료 기준은 [AUTH-05](p1/auth.md#auth-05-소셜-로그인계정-연결), 브라우저 리다이렉트와 API 계약은 [API 명세서](API.md#auth-05-소셜-로그인계정-연결)를 따른다.

~~~text
사용자가 설정된 Google·Naver·Kakao 중 하나를 선택
→ 제공자 동의와 callback의 state 검증
→ 기존 provider subject면 연결된 사용자로 로그인
→ 처음 보는 subject이고 이메일이 겹치지 않으면 새 사용자·소셜 계정을 함께 생성
→ 기존 사용자와 이메일이 같으면 자동 병합하지 않고 명시적 연결 안내
→ 기존 계정 로그인·CSRF 확인 뒤 같은 제공자를 다시 인증해 연결
→ 교체된 JSESSIONID와 새 CSRF로 보호 API 사용
~~~

### 조건으로 게임과 방 찾기

세부 검색 조건은 [P1 검색 기능 명세](p1/search.md), 요청 계약은 [API 명세서](API.md)를 따른다.
...
- 세션 TTL·직렬화 방식, 정확한 key·channel namespace, 스케줄 잠금 이름·`lockAtMostFor`와 실행시간 경고 기준은 후속 구현 이슈에서 확정한다.
...
- 중복 신청은 레코드나 순서를 바꾸지 않고 최신 `WAITING` 상태와 현재 순번을 반환한다. 취소 뒤 재신청은 같은 레코드의 순서 기준을 갱신해 대기열 맨 뒤로 이동한다.
...
| 게임 | 인원·시간 검색용 수치 컬럼과 검증 제약을 추가하고 기존 표시 필드는 유지한다. 메커니즘 목록·관계와 사용자별 해 본 게임 관계는 확장 후보를 채택한 경우에만 추가한다. |
...
| 대기 | ROOM·사용자 조합마다 단일 최신 상태 레코드를 두고 FIFO 순서와 `WAITING`, `PROMOTED`, `CANCELED`, `EXPIRED`, `ROOM_CANCELED` 결과를 보존한다. 상태 변경별 이력 레코드는 추가하지 않으며 물리 테이블·제약·인덱스는 ERD 반영 전에 확정한다. |
...
| ---: | --- | --- | --- | --- |
| 1 | 기존 API 확장 | 게임 조건 검색 | `GET /api/games` | `SEARCH-01` |
...
- 메커니즘과 `SEARCH-03` 후보의 파라미터·응답·신규 API는 후보를 채택해 P1 범위를 변경한 뒤 이 표와 API 명세서에 추가한다.

---

## 구현 완료 기준

- 일곱 가지 [핵심 사용자 흐름](#핵심-사용자-흐름)이 처음부터 끝까지 연결된다.
...
- `AUTH-05`는 세 제공자의 신규·재로그인과 명시적 연결, state 실패·취소, 중복·동시 요청, 기존 세션·CSRF와 이메일 인증 회귀를 [소셜 로그인 완료 기준](p1/auth.md#상위-통합-완료-기준)대로 검증한다.
- 약 2,000개 게임의 인원·시간·난이도 검색 데이터가 정한 규칙에 따라 적재·검증되고 기존 표시 데이터가 유지된다.
- 게임·방 단독 필터와 복합 필터가 공개 범위, 기본 정렬과 페이지네이션 규칙을 일관되게 따른다.
...
- 명시적 대기 신청·본인 상태와 순번 조회·대기 취소가 ROOM·사용자별 단일 최신 상태 레코드와 FIFO를 유지한다. 중복 신청은 레코드·순서를 바꾸지 않고 정상 응답하며, 취소 뒤 재신청은 같은 레코드의 신청 순서 기준을 갱신해 마지막 순번을 받는다.
...
- 조건부 후속 범위인 `SEARCH-01` 메커니즘 확장, `SEARCH-03`, `RANK-01`, `OPS-01`, `SEC-01`, `OPS-02`의 미구현은 P1 필수 범위 완료를 막지 않는다.
