...
`ROOM-10`은 P1 필수 범위에 그대로 두되, 그 안의 비관적 락 비교와 최종 잠금 전략 ADR은 후속으로 이관한다. P1은 현행 낙관적 락의 기준선 측정과 저장 불변식 검증까지로 완료를 판정한다. 배포가 P1 안에 완료되지 않아 실사용 동시성 계측을 확보할 시점이 P1 이후이고, 기준선은 경합을 강제한 fixture 결과라 잠금 전략 교체 여부를 판단할 입력이 되지 못하기 때문이다. 이 결정과 재검토 조건은 [#495](https://github.com/bamsongi-club/albam-mate/issues/495)에 기록했다.
