...
- 인기 랭킹은 `RANK-01`을 사용한다. [#596](https://github.com/bamsongi-club/albam-mate/issues/596)에서 P1 필수 범위로 채택해 완료 기준 ID를 부여했다.
- 조건부 후속 범위의 경계 보안은 `SEC-01`을 계속 예약하되, 범위 채택 전에는 완료 기준 ID를 부여하지 않는다. P1의 운영 관측 후보는 P2 `OPS-01`~`OPS-03`, 배포·복구 후보는 P2 `OPS-06`으로 승계한다.
...
- 170,000개 게임을 가능 인원, 플레이 시간, 권장 최소 연령, 난이도, 카테고리, 테마, 검수된 메커니즘, 추천·베스트 인원으로 찾는다.
...
운영·보안 고도화는 필수 범위를 완료한 뒤 여력이 있으면 진행하는 P1 후속 범위로 둔다. 후속 범위를 구현하지 않아도 P1 필수 범위의 완료로 판단한다.
...
P1 필수 구현은 다음 아홉 가지 흐름을 처음부터 끝까지 연결한다.
...
9. 전체·앞으로 7일 인기 게임 랭킹을 조회하고 화면에서 구분해 표시한 뒤 게임 상세로 이동하는 흐름
...
| 게임 탐색 | `SEARCH-01` | 170,000개 게임의 카테고리·테마·추천/베스트 인원 관계와 기존 인원·시간·권장 최소 연령·난이도·메커니즘의 복합 필터 제공 | [게임 조건 검색](p1/search.md#search-01-게임-조건-검색) |
...
| 영역 | 기능 ID | 후속 범위 |
| --- | --- | --- |
| 운영 관측 | P2 `OPS-01`~`OPS-03` | 요청 지연, 5xx, DB 풀과 스케줄러 실패 관측 범위를 생존·연결, 지연·포화, 실패·이상으로 나눠 승계 |
...
→ 낙관적 락 기준선 결과 제시
→ 기존 WARN 로그의 `event`·`roomId`·`attempt`로 재검토 조건 관측
→ 재검토 조건 충족 시에만 다른 잠금 전략 비교를 별도 제시
~~~
...
~~~text
사용자가 인기 게임 랭킹 화면 진입
→ `전체`와 `앞으로 7일` 랭킹을 구분해 조회
→ 로딩·빈 결과·재시도 가능한 오류 상태 확인
→ 랭킹 항목의 게임 표시 정보와 집계 모임 수 확인
→ 랭킹 항목에서 게임 상세로 이동
~~~
...
- 게임 조건 검색은 사용자가 지정한 인원이 지원 인원 범위에 포함되는지, 플레이 시간·권장 최소 연령·난이도가 선택 범위에 해당하는지를 기준으로 한다.
...
- [#495](https://github.com/bamsongi-club/albam-mate/issues/495)의 결정에 따라 비관적 락 비교는 P1에서 착수하지 않는다. 배포 후 참가 취소·자동 승격 경로에서 재시도 소진 `409`가 서로 다른 ROOM에서 반복될 때만 기존 WARN 로그의 `event`·`roomId`·`attempt`를 근거로 별도 비교를 재검토한다.
- 최종 잠금 전략의 유지·변경 판단, 근거, 트레이드오프와 재검토 조건은 비교를 다시 착수할 때 ADR에 기록한다. 별도 운영 metric은 구체적인 대시보드·알림·집계 자동화 요구가 생길 때 별도 이슈로 다룬다.
...
- 게임·방 단독 필터와 복합 필터가 공개 범위, ageBand·category·recommended/best·mechanism 내부 OR, theme ANY·ALL, 다른 조건과 AND, 기본 정렬과 페이지네이션 규칙을 일관되게 따른다.
