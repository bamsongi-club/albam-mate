...
| [ERD](ERD.md) | 현재 제공 테이블과 승인된 P1 채팅·스케줄 잠금 저장 계약, 데이터 제약과 계산식 |
...
- 채팅 기능을 활성화하는 전진 Flyway 마이그레이션은 모든 기존 방에도 채팅방을 하나씩 생성한다. 초기화와 ROOM 생성·상태 전환이 경쟁해도 완료 시점의 1:1과 상태별 보관 값이 깨지지 않아야 한다. 구체적인 동시성 제어·최종 보정과 배포 절체 방식은 후속 구현에서 확정하고, 서비스 중단이나 트래픽 차단이 필요하면 별도 OPS 승인을 받는다. 기존 최종 상태 방은 P1 메시지 이력 없이 빈 보관 완료로 초기화하고 계속 접근을 차단한다.
