...
- 170,000개 게임을 가능 인원, 플레이 시간, 최연소 참여자 나이, 난이도, 카테고리, 테마, 검수된 메커니즘, 추천·베스트 인원으로 찾는다.
...
- 게임 조건 검색은 사용자가 지정한 인원이 지원 인원 범위에 포함되는지, 최연소 참여자 나이가 게임의 권장 최소 연령 이상인지, 플레이 시간과 난이도가 선택 범위에 해당하는지를 기준으로 한다.
