...
| 게임 | 인원·시간 검색용 수치 컬럼과 검증 제약을 추가하고 기존 표시 필드는 유지한다. 사용자별 해 본 게임은 별도 관계로 저장하며, 메커니즘 목록·관계는 해당 확장 후보를 채택한 경우에만 추가한다. |
...
- 메커니즘 후보의 파라미터·응답·신규 API는 후보를 채택해 P1 범위를 변경한 뒤 이 표와 API 명세서에 추가한다.
...
- 조건부 후속 범위인 `SEARCH-01` 메커니즘 확장, `RANK-01`, `OPS-01`, `SEC-01`, `OPS-02`의 미구현은 P1 필수 범위 완료를 막지 않는다.
