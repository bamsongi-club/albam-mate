...
- 채팅 기능을 활성화하는 전진 Flyway 마이그레이션은 모든 기존 방에도 채팅방을 하나씩 생성한다. 최초 활성화는 혼합 버전 rolling 배포를 금지하고, 채팅 이전 버전의 ROOM 쓰기를 모두 중지한 뒤 PostgreSQL 쓰기 잠금 안에서 멱등 backfill과 1:1 사후조건 확인을 마쳐야 한다. 기존 최종 상태 방은 P1 메시지 이력 없이 빈 보관 완료로 초기화하고 계속 접근을 차단한다.
