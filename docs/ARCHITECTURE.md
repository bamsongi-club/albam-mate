...
App1과 `local` Nginx는 Spring의 유일한 신뢰 프록시다. HTTP와 WebSocket proxy는 외부 요청의 `X-Forwarded-For`를 이어 붙이거나 신뢰하지 않고 버린 뒤, Nginx가 직접 관찰한 `$remote_addr`로 `X-Forwarded-For`를 덮어쓴다. 따라서 Spring의 인증 요청 제한은 클라이언트가 위조한 전달 헤더가 아니라 실제 Nginx 연결 주소를 사용한다.
