...
- P2 AI provider·동의·호출 quota·고정 예약 비용·초안·확인형 Room·지역 경계: [ADR-0074](adr/platform/0074-p2-ai-provider-consent-and-operation-boundary.md), [ADR-0085](adr/platform/0085-p2-ai-quota-fixed-reservation-and-exact-game-lookup.md), [ADR-0075](adr/room/0075-p2-ai-draft-confirmation-and-idempotent-room-command.md), [ADR-0076](adr/room/0076-p2-room-region-closed-set-and-compatibility.md)
...
### P2 AI 기능군 모듈 계약 (현재 구현·승인된 확장)

> `assistant` 모듈·`assistant.contract`·`infra.ai` adapter와 AI-01·AI-03의 저장 경계는 현재 제공하며, AI-02의 provider foundation도 별도 진행 상태를 가진다. [ADR-0085](adr/platform/0085-p2-ai-quota-fixed-reservation-and-exact-game-lookup.md)의 정확 게임명 resolver·후보 DTO 확장은 승인된 목표 계약이다. 구체 제공·검증 상태는 [P2 기능 상태](p2/README.md#기능별-현재-상태)로만 판정한다. 외부 provider와 Room 쓰기 권한은 분리하고, 사용자의 명시적 확인 전에는 Room·ChatRoom 상태 변경을 허용하지 않는다.
...
AI-01~AI-03 협력 계약은 책임을 소유한 모듈의 `contract`에 둔다. provider 경계인 `AssistantIntentExtractor`, provider 기반 후보 조회와 정확 게임명 resolver는 AI-02가 각각 `assistant.contract`와 `game.contract`를 통해 소유하고, 확인형 Room 생성은 AI-03이 `room.contract`를 통해 소유한다.
...
| `AssistantExactGameNameQuery` (AI-02, ADR-0085 목표 확장) | `game.contract` | `assistant`가 호출하고 `game`이 구현 | 단독 입력과 `Game.name`을 NFKC·trim·연속 공백 축약·`Locale.ROOT` 대소문자 정규화 뒤 비교해 유일한 정식명만 `AssistantRecommendationCandidate`로 반환한다. 부분 일치·별칭·영문명·BGG ID·문장부호 제거·기본판/확장판 자동 통합은 하지 않으며 0건·복수건은 미매치다. `assistant`는 game repository·catalog를 직접 읽지 않는다. |
...
2. 정확 게임명 0건·복수건일 때만 추천의 외부 호출이 quota·비용 예약과 PII/secret allowlist 검사를 통과한 뒤 시작한다. provider 기반 추천은 후보 조회만 수행하고 Room·ChatRoom·초안을 만들지 않는다. provider 장애·schema 오류·Redis 비용 예약 실패는 명시적 오류로 끝내며 다른 model로 자동 전환하지 않는다.
...
| `assistant/contract` | AI-02 `AssistantIntentExtractor` provider 협력 계약 |
| `game/contract` (AI-02 확장) | `AssistantGameCandidateQuery`와 ADR-0085 목표의 `AssistantExactGameNameQuery` 후보 조회 계약 |
| `room/contract` (P2 확장) | AI-03 `AssistantRoomCreationCommand` 확인형 Room command와 ChatRoom 결과 handoff 계약 |
| `assistant/controller`, `assistant/dto` | AI-01 동의·AI-02 추천·AI-03 초안·확인 HTTP 경계와 요청·응답 변환 |
| `assistant/entity`, `assistant/repository` | AI-01 동의·AI-03 초안·확인 멱등성 저장 계약. provider 원문은 저장하지 않음 |
| `assistant/service` | AI-01 요청 orchestration, AI-02 추천 Provider 경계, AI-03 초안 lifecycle·확인 Executor와 트랜잭션 조정 |
...
| `infra/ai` | AI-02 `assistant.contract.AssistantIntentExtractor`의 OpenAI·fake provider adapter. `assistant` 업무 규칙은 소유하지 않음 |
