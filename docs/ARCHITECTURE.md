...
최초 채팅 활성화는 ROOM 생성·참가·취소·상태 변경과 자동 상태 보정의 진행 중인 쓰기를 drain하고 새 쓰기를 중지한 뒤 mixed-version rolling 없이 수행한다. 전진 Flyway 마이그레이션은 트랜잭션의 PostgreSQL `CURRENT_TIMESTAMP`를 하나의 초기화 경계로 사용해 아직 채팅방이 없는 모든 기존 `ROOMS`에 `CHAT_ROOMS`를 하나씩 backfill하며, 같은 INSERT를 재실행해도 기존 행과 보관 값을 변경하지 않는다. 마이그레이션 뒤 1:1·중복 부재·상태별 보관 값을 검증하고 방 생성·최종 상태 연계가 포함된 신버전을 모든 인스턴스에 배치한 뒤에만 ROOM 쓰기와 채팅 API를 활성화한다. 검증 실패나 구버전 롤백 중에는 ROOM 쓰기를 재개하지 않는다. 신버전으로 다시 절체해 같은 검증을 통과한 뒤에만 ROOM 쓰기를 재개하며, 실제 운영 쓰기 중지와 절체에는 사용자·OPS 승인이 필요하다.
