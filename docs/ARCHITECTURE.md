...
```mermaid
flowchart LR
    waitlistController["대기 Controller"] --> waitlistCommand["대기 등록 CommandService<br/>트랜잭션 없음"]
    waitlistCommand --> waitlistCoordinator["PART-04 전용 Coordinator<br/>request time 고정·총 3회"]
    waitlistCoordinator --> waitlistExecutor["대기 등록 Executor<br/>REQUIRES_NEW"]
    waitlistExecutor --> waitlistRepositories["Room·Participation·Waitlist Repository"]
```
...
`V6__create_p1_chat_room_schema.sql`은 `CHAT_ROOMS` 테이블·제약만 생성하며 기존 `ROOMS`를 조회하거나 `CHAT_ROOMS` 행을 삽입·갱신하지 않는다. `V11__create_p1_chat_retention_schema.sql`은 #289의 로컬·초기화 검증 범위에서 `CHAT_ROOMS`가 없는 기존 ROOM만 상태별 보관 값으로 생성하며 이미 있는 행은 덮어쓰지 않는다. [#279의 최신 승인 테스트 계약](https://github.com/bamsongi-club/albam-mate/issues/279#issuecomment-5161788285)은 live 운영 backfill·상태별 초기화·ROOM 생성·상태 전환 경합·최종 보정·배포 절체를 [#281](https://github.com/bamsongi-club/albam-mate/issues/281)의 후속 범위로 분리한다. [ADR-0045](adr/chat/0045-chat-room-schema-and-backfill-boundary.md)은 이를 명시적 one-shot/maintenance 작업으로 수행하는 경계를 제안하며 팀 채택 전에는 승인된 실행 계약이 아니다. 따라서 일반 애플리케이션의 Flyway 실행에도 V11의 로컬·초기화 검증 범위는 포함되지만, 이것이 #281의 live 운영 절체 계약을 대체하지 않는다.
