...
상세 조회는 `RoomController`, 참가·참가 취소는 `RoomParticipationController`가 담당한다. package-private인 `RoomQueryParameterAllowlistValidator`는 허용 목록 밖의 query parameter 이름을 거부한다.
...
| `RoomCommandExecutionCoordinator` | package-private | 요청 시각 고정과 재시도 순서 조정 |
| `RoomOptimisticLockRetrier` | public·사용자 제한 | 낙관 락 재시도와 오류 변환만 담당하며 시간·업무 규칙은 제외 |
...
`RoomCommandExecutionCoordinator`는 요청 시각을 고정하고 낙관 락 충돌만 재시도한다. 각 Executor 시도는 Spring Proxy를 거친 `REQUIRES_NEW` 트랜잭션에서 최신 Entity와 version을 다시 조회한다.
