...
- `AC1` 성공·실패 응답의 필드 구성과 `status` 값이 공통 응답 계약과 일치한다.
- `AC2` API 명세 9절의 모든 오류 코드를 코드에서 참조할 수 있고, 예외 클래스명이나 로그 메시지로 코드를 즉석에서 만들지 않는다.
- `AC3` 검증 실패, 인증·권한 실패와 처리하지 않은 예외가 각각 계약된 오류 코드로 변환된다.
- `AC4` 실패 응답에 비밀번호, 세션 식별자, 인증정보와 사용자 ID가 포함되지 않는다.
- `AC5` 위 변환 경로의 성공·실패 테스트가 있고 CI를 통과한다.
...
- `AC1` 애플리케이션 코드가 시스템 기본 시간대에 의존하지 않고 주입받은 `Clock`으로 현재 시각을 얻는다.
- `AC2` 오프셋이 다른 같은 순간의 요청 값이 같은 시각으로 저장·비교된다.
- `AC3` 응답의 시각 필드가 `+09:00` 오프셋으로 반환된다.
- `AC4` `Clock.fixed(...)`로 고정 시각을 주입하는 테스트가 있고 CI를 통과한다.
...
- `AC1` 빈 데이터베이스에 초기 마이그레이션이 적용되고 재실행에서 이력·체크섬이 유지된다.
- `AC2` 엔티티의 식별자 타입·생성 전략과 시각 필드 타입이 연결한 ADR과 ERD 제약에 일치한다.
- `AC3` 도메인 패키지가 컨벤션의 참조 방향을 위반하는 import 없이 컴파일된다.
- `AC4` 컨벤션의 패키지 트리와 실제 생성한 패키지 목록이 일치한다. 어긋나면 같은 PR에서 문서를 맞춘다.
- `AC5` 공유 개발·검증·운영 환경에서 Hibernate가 스키마를 생성·변경하지 않는다.
...
- `AC1` 공개 경로는 비로그인 요청을 허용하고 보호 경로는 인증 없는 요청을 계약된 오류로 거절한다.
- `AC2` 세션 쿠키와 CSRF 토큰의 속성·전달 방식이 API 계약과 일치한다.
- `AC3` 업무 모듈이 인증 사용자 식별을 `global`이 공개한 계약으로만 얻는다.
- `AC4` 인증·권한 실패 응답에 세션 식별자와 인증정보가 포함되지 않는다.
- `AC5` 공개·보호 경로와 CSRF 처리의 성공·실패 테스트가 있고 CI를 통과한다.
...
- `AC1` 비밀번호는 `PasswordEncoder` 계약으로만 저장·검증되고 원문이나 빠른 단일 해시를 저장하지 않는다.
- `AC2` 같은 원문의 저장 값이 요청마다 다르고 알고리즘 식별자를 포함한다.
- `AC3` 허용량과 동시 작업 한도를 넘는 요청은 해시 작업 없이 계약된 오류와 `Retry-After`를 반환한다.
- `AC4` 제한 경계, 만료와 초기화 동작의 테스트가 있고 CI를 통과한다.
- `AC5` work factor와 동시 작업 한도를 운영 유사 환경에서 측정한 근거를 남긴다. 측정하지 않은 값을 검증 완료로 표현하지 않는다.
...
- `AC1` PostgreSQL 컨테이너에서 초기 마이그레이션이 적용되고 스키마 검증이 통과한다.
- `AC2` PostgreSQL 검증을 H2 테스트와 분리해 실행할 수 있고 실행 명령이 문서에 있다.
- `AC3` CI가 이 검증을 실행하고 실패를 드러낸다.
- `AC4` PostgreSQL 전용 SQL·제약과 동시성 동작을 H2 테스트 결과로 대체하지 않는다.
...
- `AC1` 모듈 간 순환 의존이 있으면 구조 테스트가 실패한다.
- `AC2` 다른 모듈의 `repository`, `entity` 또는 HTTP DTO를 직접 참조하면 구조 테스트가 실패한다.
- `AC3` 허용 의존 관계가 컨벤션의 참조 방향과 일치한다. 어긋나면 같은 PR에서 문서와 테스트를 맞춘다.
- `AC4` 구조 테스트가 CI에서 실행된다.
