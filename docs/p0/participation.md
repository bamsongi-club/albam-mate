...
- 성공 시 `200 OK`와 `participationStatus = CANCELED`를 반환한다.
- 시작 전 취소로 빈자리가 생기면 공통 상태 규칙에 따라 방이 `RECRUITING`으로 복귀한다.
- 주최자 요청, 활성 참가 관계가 없는 요청과 시작 이후 요청이 각각 API 계약의 오류로 거절된다.
- 참가와 취소가 동시에 실행돼도 참가 수와 방 상태가 ERD·ADR 불변식을 만족한다.
...
- `joined`, `hosted`, `all`이 각각 정의된 범위만 반환한다.
- 참가 취소 관계와 취소된 참가 방은 `joined`에서 제외하고 `FINISHED` 방은 포함한다.
- 상태 보정이 완료된 결과를 기준으로 필터와 페이지를 계산한다.
- 내 모임 항목에 정확한 장소와 전체 참가자 목록이 노출되지 않는다.
