...
- `AC1` 신규 참가와 재참가는 모두 `201 Created`와 `participationStatus = ACTIVE`를 반환한다.
- `AC2` 마지막 좌석 참가 시 방이 `CLOSED`가 된다.
- `AC3` 방이 정원 충족으로 `CLOSED`인 동안, 주최자도 현재 `ACTIVE` 참가자도 아닌 사용자의 신규 참가 요청은 `CAPACITY_EXCEEDED`를 반환한다.
- `AC4` 방이 `CANCELED` 또는 `FINISHED`로 전이된 뒤의 참가 요청은 `ROOM_NOT_RECRUITING`을 반환한다.
- `AC5` 동일 사용자의 활성 참가 관계가 중복 생성되지 않는다.
- `AC6` 실제 PostgreSQL 동시 요청에서도 모집 정원을 초과하지 않고 충돌 결과가 API·ADR 계약과 일치한다.
...
- `AC1` 성공 시 `200 OK`와 `participationStatus = CANCELED`를 반환한다.
- `AC2` 시작 전 취소로 빈자리가 생기면 공통 상태 규칙에 따라 방이 `RECRUITING`으로 복귀한다.
- `AC3` 주최자 요청, 활성 참가 관계가 없는 요청과 시작 이후 요청이 각각 API 계약의 오류로 거절된다.
- `AC4` 참가와 취소가 동시에 실행돼도 참가 수와 방 상태가 ERD·ADR 불변식을 만족한다.
...
- `AC1` `joined`, `hosted`, `all`이 각각 정의된 범위만 반환한다.
- `AC2` 참가 취소 관계와 취소된 참가 방은 `joined`에서 제외하고 `FINISHED` 방은 포함한다.
- `AC3` 상태 보정이 완료된 결과를 기준으로 필터와 페이지를 계산한다.
- `AC4` 내 모임 항목에 정확한 장소와 전체 참가자 목록이 노출되지 않는다.
