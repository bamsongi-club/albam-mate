...
- `ROOM-01-AC1` 필터 없는 조회는 두 유형의 `RECRUITING`, `CLOSED` 공개 방을 반환한다.
- `ROOM-01-AC2` `type`, `gameId`, `keyword`의 각 단독 필터와 조합 필터는 전달된 조건을 모두 만족하는 공개 방만 반환한다.
- `ROOM-01-AC3` 공개 목록은 시작 시각, 방 ID 오름차순과 페이지네이션을 유지하며 개인정보를 포함하지 않는다.
- `ROOM-01-AC4` 상태 정합화가 끝난 현재 공개 대상만 목록에 포함하며 `joinable`은 요청자 기준으로 계산한다.
- `ROOM-01-AC5` 비로그인 사용자도 방 목록을 조회할 수 있다.
