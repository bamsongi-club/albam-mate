...
재시도 소진율이 뚜렷하게 증가하기 시작하는 수준은 시나리오마다 다르다.

| 시나리오 | 소진율 (수준 2 / 4 / 8) | 증가 시작 수준 |
| --- | ---: | --- |
| room-10a 마지막 좌석 동시 참가 | 0% (0/6) / 0% (0/12) / 0% (0/24) | 측정한 `2`·`4`·`8` 안에서 관찰되지 않음 |
| room-10b 참가 취소·자동 승격 | 0% (0/30) / 25.0% (3/12) / 54.2% (13/24) | 수준 `4`부터 나타나 수준 `8`에서 크게 증가 |

이 수준은 합격·불합격 판정이 아니라 현재 재시도 상한인 최초 1회와 재시도 최대 2회에서 관찰된 한계 기록이다.

두 시나리오가 갈리는 지점은 재시도 bucket에서 관찰된다. room-10a는 수준 `2`·`4`·`8` 모두 `2`회 재시도가 `0`이라 세 번째 시도에 도달한 요청 자체가 없다. 충돌한 요청이 두 번째 시도에서 정원 초과를 확인하고 업무 실패로 끝나므로 이 fixture에서는 소진이 관찰될 수 없다. 반면 room-10b는 수준 `4`에서 `2`회 재시도 `6`건 중 `3`건, 수준 `8`에서 `17`건 중 `13`건이 소진됐다.

수준 `2`의 room-10b 30개 요청은 다섯 시나리오를 합친 값이므로 수준 `4`·`8`과 같은 분포로 비교하지 않는다. 두 보고서의 수준 집합은 `2`·`4`·`8`로 같지만 시나리오 구성이 다르므로, 위 증가 시작 수준은 각 시나리오 안에서만 의미를 가진다.

## 후속 결정 경계

ROOM-10-AC5와 AC6의 잠금 전략 결정은 [#495](https://github.com/bamsongi-club/albam-mate/issues/495)에서 확정됐다. P1에서는 현행 낙관적 락을 유지하고 비관적 락 비교와 최종 잠금 전략 ADR을 시작하지 않는다. 배포 후 참가 취소·자동 승격 경로에서 재시도 소진 `409`가 서로 다른 ROOM에서 반복되면, `RoomOptimisticLockRetrier`의 기존 WARN 로그에 남는 `event`·`roomId`·`attempt`를 관측 근거로 동일 환경·동일 시점의 별도 비교를 재검토한다. 별도 운영 metric은 대시보드·알림·집계 자동화 같은 구체적인 요구가 생길 때 별도 이슈로 재검토한다. 이 기준선 문서는 그 비교 구현을 시작하지 않는다.

이 문서의 절대 수치는 로컬 단일 인스턴스와 Testcontainers 환경에서 얻은 값이므로, 배포 환경에서 다시 측정한 값과 직접 비교하지 않는다. 잠금 전략을 비교할 때는 두 후보를 **같은 환경에서 같은 시점에** 함께 실행하고 그 실행 안에서만 비교한다. 오늘의 낙관 락 수치를 나중에 다른 환경에서 측정한 비관 락 수치의 상대편으로 쓰지 않는다는 뜻이다.

따라서 이 기준선에서 다음 비교로 이전되는 것은 절대 수치가 아니라 두 가지다. 하나는 결정적 read gate·확정 순서 gate·fixture 비용 분리·재시도 bucket 산식·원자료 JSON 형식을 포함한 측정 하네스이고, 다른 하나는 경합이 집중되는 지점이다. 후자는 하드웨어가 아니라 코드 경로의 성질에서 나오므로 환경이 바뀌어도 유지된다. 반면 수준 `2`·`4`·`8` 사이의 소진율 곡선은 같은 환경에서 얻은 값끼리의 비교이므로 이 문서 안에서는 그대로 유효하다.
