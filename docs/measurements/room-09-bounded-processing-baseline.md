...
소형 fixture의 due ROOM 20개는 `RECRUITING` 10개와 `CLOSED` 10개로 구성된다. `WAITING`은 `CLOSED` due ROOM에만 ROOM당 10명씩 있으므로, 각 run은 `CLOSED` due ROOM 10개 × ROOM당 `WAITING` 10명 = 총 100건을 종료했다. 표의 후보 수와 성공 수 `20`은 `WAITING` 행 수가 아니라 처리한 due ROOM 수다. `ROOM-09d-T3` 계약에 따라 이 결과를 현행 대비 성능 개선 비교에 넣지 않으며, `WAITING` 없는 후보 결과와 견주지도 않는다. 두 값은 fixture 구성이 다르고 서로 다른 실행에서 얻어, 차이를 대기열 유무의 효과로 읽을 수 없다. 대기열 종료 비용을 분리하려면 통제된 비교 측정 계약을 따로 승인받아야 한다.
