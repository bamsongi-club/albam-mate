...
`exit=2`인 3종의 위반 지표다. **직전 after와 위반 지표 구성은 같지만 수치는 다르다.**
...
전송 성공 요청의 저장과 재연결 후 이력 복구는 이번 Run에서 직접 검증하지 않았다. 따라서 이 자료만으로 메시지 보존·복구를 확정하지 않는다.
...
- **재연결 후 이력 복구를 직접 검증하지 않았다.** 전달 누락이 메시지 보존·복구로 이어지는지는 별도 검증이 필요하다.
