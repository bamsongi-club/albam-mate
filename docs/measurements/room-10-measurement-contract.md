# ROOM-10a·10b 동시성 기준선 측정 계약

## 목적과 범위

이 문서는 현재 `RoomOptimisticLockRetrier`를 사용하는 마지막 좌석 동시 참가(`ROOM-10a`)와 대기·자동 승격 경합(`ROOM-10b`)의 기준선을 재현하기 위한 fixture와 원자료 형식을 고정한다. 결과는 성능 합격선이나 운영 실측이 아니며, 비관적 락·조건부 갱신·연결 풀 처리량 또는 다른 잠금 전략을 비교하거나 평가하지 않는다.
