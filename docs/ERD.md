...
- 요청 생성과 전원 수락 최종 확정은 사용자별 lifecycle을 직렬화하는 PostgreSQL 트랜잭션이다. 요청 생성은 대상 `USERS` 행 하나를, 전원 수락 최종 확정은 고정 제안 회원의 모든 `USERS` 행을 `id ASC` 결정적 순서로 `FOR UPDATE` 잠근다. 잠금 뒤 요청 생성은 `WAITING`·`PROPOSED`·`PAUSED` 현재 요청과 `PREPARING` Party의 참가자 관계, `ACTIVE` Party의 `left_at IS NULL` 참가자 접근 관계가 모두 없는지 확인한다. 전원 수락 최종 확정은 각 사용자의 현재 요청이 그 제안의 `PROPOSED` 요청 하나뿐이고 같은 현재 Party 관계가 없는지 함께 확인한다. 그 뒤 요청을 `MATCHED`로 바꾸고 파티·참가자 접근 관계를 원자적으로 만든다. `CLOSED`와 명시적으로 나간 `left_at IS NOT NULL` 관계는 이 판정에서 제외해 한 사용자에게 UI의 현재 상태가 하나만 남도록 한다.
- 서버 시작과 주기 recovery는 `PREPARING` 파티도 다시 선별한다. scan·ShedLock은 후보 중복 조회만 조정하므로 Party별 Executor는 시작 즉시 대상 `MATCH_PARTIES` 행을 `FOR UPDATE`로 잠근다. 잠금 뒤 PostgreSQL 시각으로 `operation_time`과 `preparing_started_at + 5분` deadline을 정하고 현재 `status`, MATCH chat 존재를 다시 읽는다. deadline 전의 아직 `PREPARING` Party만 provision 또는 기존 채팅 재사용을 시작할 수 있으며, `PREPARING → ACTIVE` 조건부 갱신은 직전 DB 시각도 deadline 전인 경우 영향 행이 1일 때만 성공한다. 그 조건이 0건이거나 lock 뒤 deadline이 지났으면 채팅방 존재와 무관하게 아직 `PREPARING`인 Party의 실패 cleanup만 수행한다. cleanup도 같은 상태·기한 조건에서만 실행하고, 잠금 뒤 상태가 바뀌었거나 Party가 사라졌으면 no-op으로 끝내므로 ACTIVE 전이와 실패 삭제가 같은 Party에 함께 적용되지 않는다. 준비 시작 후 5분까지 `ACTIVE`로 회복하지 못하면 chat 정리 경계가 부분 `MATCH_CHAT_ROOMS`를 먼저 삭제하고 그 내부 메시지를 cascade한 뒤, 성공 파티·참가자 접근 관계를 물리 삭제하고 연결 요청을 기존 `queued_at`·`priority_since`를 보존한 `WAITING`으로 복귀시킨다.
...
- `PREPARING`과 `CLOSED`는 일반 사용자 메시지 조회·전송·실시간 구독을 허용하지 않는다. `ACTIVE → CLOSED` 뒤 성공 파티·참가자 접근 관계와 MATCH 채팅은 즉시 비공개가 되며 실제 `closed_at`부터 7일 보존한다. party retention 시 chat 정리 경계가 `MATCH_CHAT_ROOMS`를 먼저 삭제하고 그 내부 URL 텍스트를 포함한 메시지를 cascade한 뒤, `MATCH_PARTY_PARTICIPANTS`와 `MATCH_PARTIES`를 삭제한다. `MATCH_CHAT_ROOMS.party_id`의 `ON DELETE RESTRICT`는 matching 소유 파티 삭제가 chat을 연쇄 삭제하지 못하게 한다. 이 삭제가 끝난 뒤 `PURGED` 행을 남기지 않는다.
