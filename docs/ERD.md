...
이 문서는 현재 제공 중인 P0와 승인된 P1 채팅·다중 인스턴스 스케줄 잠금의 데이터 모델·데이터 제약을 정의한다. P1 항목은 구현 예정 계약이며 Flyway 마이그레이션과 코드가 반영되기 전에는 현재 운영 스키마로 보지 않는다.
...
- 기준: P0 제품 규칙은 [P0 공통 명세](archive/p0/P0-spec.md), P1 채팅·스케줄 규칙은 [P1 명세](P1-spec.md)와 [관련 승인 ADR](adr/README.md)을 따른다.
- 범위: 현재 P0의 오프라인 방·게임 목록·사용자·방 참가, P1 구현 예정인 방별 채팅과 공용 스케줄 잠금
- 제외: 아직 저장 계약을 확정하지 않은 P1 검색·대기·알림 구조, 온라인 방, 온라인 자동 매칭, 후기, 룰마스터 가능 게임, 결제·포인트
