...
P1 채팅방을 저장하는 구현된 테이블이다. `V6__create_p1_chat_room_schema.sql`은 테이블·제약만 만들고 기존 `ROOMS`를 조회하지 않았다. `V11__create_p1_chat_retention_schema.sql`은 #289의 로컬·초기화 검증 범위에서 `CHAT_ROOMS`가 없는 기존 ROOM만 상태별 보관 값으로 생성한다. 기존 행을 덮어쓰지 않으며, 이 자동 전진 Flyway는 [#281](https://github.com/bamsongi-club/albam-mate/issues/281)의 live 운영 one-shot backfill·ROOM 쓰기 통제·최종 보정·배포 절체를 대체하지 않는다. [ADR-0045](adr/chat/0045-chat-room-schema-and-backfill-boundary.md)은 명시적 one-shot/maintenance 작업 경계의 제안이며 팀 채택 전에는 승인된 실행 계약이 아니다.
...
- `V11`은 로컬·초기화 검증에서 기존 `CHAT_ROOMS`가 없는 ROOM만 생성한다. `RECRUITING`·`CLOSED`는 `NULL` 보관 값, `CANCELED`·`FINISHED`는 하나의 PostgreSQL 기준 시각의 빈 보관 완료 값을 쓴다. 기존 행은 덮어쓰지 않는다. live 운영 backfill·ROOM 생성·상태 전환 경합·최종 보정과 배포 절체는 [#281](https://github.com/bamsongi-club/albam-mate/issues/281)의 별도 범위이며, [ADR-0045](adr/chat/0045-chat-room-schema-and-backfill-boundary.md)을 승인 상태로 바꾸지 않는다.
