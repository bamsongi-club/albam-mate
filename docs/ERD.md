...
| session_id | BIGINT | FK → SESSIONS.id, NN | 대상 세션 |
...
| SESSIONS | CHECK (capacity BETWEEN 1 AND 10) | 개설자를 제외한 모집 정원은 1명 이상 10명 이하다. |
...
- 세션 수정은 `status = RECRUITING`, `now < start_at`, 개설자 외 `ACTIVE` 참가자 없음 조건을 모두 만족할 때만 할 수 있다.
...
- `now >= start_at + 24시간`일 때 `CLOSED` 세션은 `FINISHED`로 전환한다.
