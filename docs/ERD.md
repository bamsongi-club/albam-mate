...
| 변경 시 함께 갱신 | 스키마를 바꾸면 Flyway 마이그레이션과 JPA 엔티티를 같은 변경에서 일치시킨다(→ [CONVENTIONS](CONVENTIONS.md#데이터베이스-변경), [ADR-0008](adr/platform/0008-flyway-database-migrations.md)) |
