---
name: second-opinion
description: "코드 또는 GitHub PR을 read-only로 다시 리뷰할 때 사용한다. Codex `review-code` 1차 리뷰 뒤 다른 관점의 2차 리뷰가 필요한 경우를 기본 맥락으로 삼고, 관련 차원을 최대 3개 선택해 second-opinion-reviewer 서브에이전트로 병렬 검토한다. PR 번호나 URL과 함께 리뷰를 요청하면 검증된 결과를 해당 PR에 게시한다. 트리거: '2차 리뷰', '다른 관점으로 봐줘', '한 번 더 봐줘', '리뷰해줘', '코드 리뷰', 'N번 PR 리뷰해줘', '병렬 리뷰', '컨벤션 봐줘', '/second-opinion'."
---

이 문서는 Claude Code 전용 독립본이다. Codex의 1차 리뷰 스킬은 `review-code`(`.agents/skills/review-code/SKILL.md`)이고, Claude Code의 2차 리뷰 스킬은 이 문서의 `second-opinion`이다. 두 스킬은 역할이 다르므로 이름도 다르게 두며, 같은 이름의 사본으로 취급하지 않는다. 리뷰 방법론은 같지만 실행 엔진 절이 다르므로 서로 복사해 맞추지 않고, 리뷰 기준을 바꿀 때는 양쪽을 각각 고친다.

## 이 스킬의 위치

팀 기본 흐름은 Codex `$review-code` 1차 리뷰다. 이 스킬은 1차 결과가 애매하거나 다른 관점의 확인이 필요할 때 쓰는 2차 리뷰다.

- 1차 리뷰 결과를 받았더라도 서브에이전트 프롬프트에 넣지 않는다. 같은 결론을 복창하게 만들면 2차 리뷰의 의미가 없다. 1차 결과는 오케스트레이터가 차원 선택과 최종 대조에만 쓴다.
- 1차에서 이미 다룬 차원을 확인할 수 있으면 그 차원은 자동 선택에서 내리고 아직 보지 않은 렌즈를 우선한다. 사용자가 같은 차원의 재확인을 명시하면 그대로 실행한다.
- 1차 결과와 상충하는 결론은 감추지 않는다. 판정기로 재확인한 뒤 최종 보고에 `1차와 상충`을 표시하고 근거를 남긴다.
- 1차 리뷰가 이미 PR에 게시됐을 수 있으므로 기본은 텍스트 보고다. 게시 모드에서는 review body 첫 줄에 2차 리뷰임을 밝힌다.
- 사용자가 1차 리뷰를 언급하지 않으면 일반 리뷰로 처리한다. 없는 1차 결과를 찾아다니지 않는다.

## 역할과 불변 규칙

- 요청한 변경 또는 파일을 차원별 전문 리뷰 패스로 검토한다.
- 코드·설정·문서를 수정하거나 요청하지 않은 테스트를 작성하지 않는다.
- 메인 대화가 오케스트레이터다. 범위, 위험 매니페스트, 공통 검증, 중복 제거, 최종 보고와 PR 게시를 오케스트레이터가 맡는다.
...
차원별 패스는 `Agent` 도구로 만든다.

| 항목 | 사용 값 |
| --- | --- |
| 스캐너 | `subagent_type: second-opinion-reviewer` |
| 판정기 | `subagent_type: second-opinion-judge` |
| 병렬 실행 | 한 응답에 여러 Agent 호출을 함께 넣는다. 호출을 나눠 보내면 순차 실행된다. |
| 모델 | 두 에이전트 정의의 기본값은 opus다. 소형·저위험 diff에서 슬롯을 넓혀야 하면 호출 시 `model: sonnet`으로 내리고 최종 보고에 남긴다. |
| read-only 보장 | 두 에이전트 정의의 `tools`에 Edit·Write·Bash가 없다. 프롬프트로 금지하는 대신 도구 자체를 주지 않는다. |

- 서브에이전트에 git 실행 도구가 없으므로 diff hunk, 파일 구간, 동반 문맥을 오케스트레이터가 프롬프트에 직접 주입한다. "직접 확인하라"고 넘기지 않는다.
- Codex와 달리 실행 중인 서브에이전트에 상태 반환을 요청하거나 중간에 중단할 수 없다. 시간 기반 soft·hard limit 대신 spawn 전에 샤드 크기를 제한하고, 프롬프트에 "후보 3개를 확보하면 즉시 반환"을 자기 제한으로 넣는다.
- 세션 정책이나 권한 설정으로 Agent 도구를 쓸 수 없으면 메인 에이전트가 같은 차원 잠금·같은 입력으로 순차 리뷰하고, 서브에이전트 없이 실행했다는 사실을 최종 보고에 남긴다.
...
| 조건 | 행동 |
| --- | --- |
| 샤드당 후보 상한 | 3개. 확보하면 즉시 반환한다. |
| 샤드 범위 상한 | 파일 8개, 추가·삭제 800줄, 단일 파일 600줄 중 하나를 넘으면 클래스·함수·API 절 같은 논리 경계로 나눈다. |
| 동시 실행 | 한 배치에 최대 3개 샤드를 함께 spawn하고 나머지는 다음 배치로 넘긴다. |
| complete=false 반환 | 미검토 위험만 절반 이하 크기로 재분할한다. 재분할은 두 단계까지만 한다. |
...
- 오케스트레이터가 대상 파일, stat, PR 설명, 핵심 hunk 또는 지정 파일 내용을 보고 자동 차원을 1~3개 고른다. 차원 선택에는 서브에이전트를 쓰지 않는다. 각 차원에는 파일 또는 hunk 근거를 한 줄로 남긴다.
...
- 2차 리뷰에서 1차가 다룬 차원을 확인할 수 있으면 동점 후보 중 1차가 보지 않은 차원을 먼저 올린다.
- 실행 전에 선택 결과와 근거를 알린다. 선택하지 않은 차원의 서브에이전트는 만들지 않는다.

### 차원별 샤딩

- 지정 파일 리뷰에서는 파일 전체 또는 필요한 파일 구간과 최소 호출자·테스트를 함께 제공한다. 동반 파일은 테스트·설정·정책·공개 계약으로 한정한다.
- diff 대상은 실제 hunk와 전후 최소 5줄을 준다. 전체 patch나 전체 파일 목록을 모든 패스에 반복 전달하지 않는다.
- 차원별 파일 분배는 다음을 기준으로 한다.

| 차원 | 전달 대상 |
| --- | --- |
| correctness, behavioral-correctness | 실행 코드, 설정, 스크립트와 해당 테스트 |
| security, privacy | 인증·인가·입력·로그·비밀정보·외부 명령·공개 응답 경계 |
| performance, cpu-perf-patterns | 쿼리·반복·동시성·대량 처리 경로 |
| cross-file-consistency | 함께 바뀐 API·DTO·스키마·명세·ADR 계약 파일 |
| architecture, conventions, test-coverage | 각 차원의 근거가 있는 모듈·규칙·테스트 파일 |
...
- `develop → main` 같은 릴리스 리뷰에서 이전 검증 SHA가 현재 이력의 조상으로 확인되면 그 이후 변경과 통합 위험 hunk를 우선 전달한다. 확인할 수 없으면 전체 범위로 폴백한다.
...
각 Agent 프롬프트에 다음을 주입한다.

- **리뷰 범위**: 고정 base/head SHA 또는 파일 스냅샷, 대상 종류, shard ID, 위험 ID, 정확한 파일 구간 또는 hunk와 최소 문맥, 동반 파일, 외부 계약 요약.
- **차원 잠금**: "너는 오직 `<차원>`만 본다. 다른 차원 문제는 무시하라."
- **범위 잠금**: 주입한 구간과 동반 파일 밖으로 Read·Grep을 넓히지 말라고 명시한다.
- **크기 제한**: 대형 diff면 minor·nit을 탐색하지 말라고 명시한다.
- **컨벤션 소스**(conventions 한정): AGENTS.md 또는 CLAUDE.md가 가리키는 정본, docs/CONVENTIONS.md, CONTRIBUTING.md, .editorconfig, 린터·포매터 설정 순으로 기준을 찾는다. 정본이 없으면 언어 관용을 쓴다.
- **반환 형식**: 아래 출력 절의 JSONL만 반환하게 한다.

### 3. 병합과 재판정

- 같은 위치·같은 문제를 하나로 합치고 coverage 장부의 미검토 범위를 먼저 처리한다.
- 추가·수정·문맥 라인은 head의 해당 줄과 문맥으로, 삭제 라인은 preimage와 LEFT diff 문맥으로 다시 검증한다. 잘린 출력이나 대상 범위 밖 후보는 채택하지 않는다.
- 외부 계약이 필요한 후보는 공식 1차 출처로, 보안·개인정보·critical·major·상충·medium confidence·1차 리뷰와 상충하는 후보는 판정기로 재확인한다. 판정기에는 전체 diff를 주지 않고 후보와 근거 hunk, 계약 요약만 준다.
- 위치·실패 시나리오·계약 근거가 없거나 low confidence인 후보는 제외한다. 검증된 Finding만 최종 형식으로 확장한다.
- 차원 간 상충(예: 성능 대 가독성)은 트레이드오프로 명시한다.

## GitHub PR 게시

게시 모드에서만 병합이 끝난 Finding을 한 번의 COMMENT review로 보낸다. PR 전체 요약을 review body로, 각 Finding을 해당 diff 라인의 inline comment로 보낸다.

- 최신 headRefOid를 commit_id로 쓴다. 추가·수정·문맥 라인은 line과 side=RIGHT, 삭제 라인은 preimage의 line과 side=LEFT를 쓴다. 폐기 예정인 position 방식은 쓰지 않는다.
- diff에 안전하게 앵커할 수 없는 Finding은 inline으로 억지로 보내지 않고 PR 전체 요약에 file:line으로 남긴다.
- payload에는 commit_id, body, event=COMMENT, comments의 path, line, side, body를 넣고 `gh api --method POST -H "Accept: application/vnd.github+json" "repos/$repo/pulls/$pr/reviews" --input <payload-file>`로 전송한다. Finding이 없으면 comments를 생략하고 요약만 게시한다. event는 판정과 무관하게 항상 COMMENT다. APPROVE나 REQUEST_CHANGES로 GitHub의 승인·변경 요청 상태를 바꾸지 않는다.
...
- 대형 diff가 아닐 때만 minor·nit을 보고한다. 샤드는 잘된 점, 장문 설명, 전체 코드, PR 요약을 반환하지 않는다.
...
- 1차 리뷰와 상충하는 Finding은 제목 뒤에 `(1차와 상충)`을 붙이고 문제점 절에 어느 쪽이 왜 맞는지 남긴다.
...
- 2차 리뷰를 게시하는 경우 요약 첫 줄에 어떤 관점으로 다시 봤는지 한 줄로 밝힌다.
