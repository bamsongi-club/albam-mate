...
- 먼저 `.codex/agents/backend-developer.toml`을 읽고 `developer_instructions` 문자열의 지시를 그대로 따른다. 그 파일이 지시 정본이므로 이 문서에 요약본이나 사본을 두지 않는다.
- 정본을 읽지 못하면 구현하지 않고 그 사실과 원인만 반환한다.
- `.toml`의 `model`·`sandbox_mode`는 Codex 실행 설정이므로 따르지 않는다. 실행 설정은 이 파일의 frontmatter를 쓴다.
