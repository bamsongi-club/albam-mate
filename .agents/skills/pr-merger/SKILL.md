---
name: pr-merger
description: "작성자 본인이 명시적으로 요청한 GitHub PR을 승인 리뷰·CI·위험도 기반 diff 퀴즈와 필요시 변경 요약 설명·확인으로 이해도를 검증한 뒤 squash merge하고 head 브랜치를 삭제한다. 트리거: 'PR #5 머지해줘', '리뷰 끝난 PR 합쳐줘'."
---
...
## 3. Diff 이해도 확인 — 위험도 기반 퀴즈·변경 요약 설명 및 확인
...
- 선택한 모든 개념을 퀴즈로 맞히거나 재시험 후 변경 요약을 이해하고 머지에 명시적으로 동의하면 이해도 게이트를 통과한다. 사용자 대신 답하거나 변경 요약을 제공하지 않은 단순 확인으로 통과시키지 않는다.
